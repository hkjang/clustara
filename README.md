...
| **Pod 관리** | Pod 목록·상세, 컨테이너 상태, 이벤트, 현재/previous 로그 조회, 로그 분석·조치 추천, 실시간 tail, 증적 번들, Golden Pod Diff, Health Replay, 검색·마스킹·다운로드·감사 |
| **터미널 정책** | Pod exec/web terminal 사전 정책: role·namespace·label·명령 allow/deny·승인·세션 시간·감사 평가 |
