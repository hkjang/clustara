...
func buildSettingRegistry() []settingDef {
	...
	semanticScopeValue := func(v string) error {
		switch strings.ToLower(strings.TrimSpace(v)) {
		case "global", "team", "key":
			return nil
		}
		return fmt.Errorf("must be one of global, team, key")
	}
	...
	return []settingDef{
		...
		{Key: "cache.chat_semantic_scope", Category: "cache", Type: stString, validate: semanticScopeValue, envValue: func(c config.Config) string { return c.Cache.ChatSemanticScope }},
		...
		{Key: "k8s.services.jupyterhub_idle_threshold_minutes", Category: "k8s.services", Type: stInt, validate: boundedInt(5, 10080), envValue: func(config.Config) string { return envOr("K8S_SERVICE_JUPYTERHUB_IDLE_THRESHOLD_MINUTES", "60") }},
		{Key: "k8s.services.jupyterhub_http_timeout_seconds", Category: "k8s.services", Type: stInt, validate: boundedInt(2, 120), envValue: func(config.Config) string { return envOr("K8S_SERVICE_JUPYTERHUB_HTTP_TIMEOUT_SECONDS", "10") }},
		{Key: "k8s.services.jupyterhub_user_limit", Category: "k8s.services", Type: stInt, validate: boundedInt(1, 2000), envValue: func(config.Config) string { return envOr("K8S_SERVICE_JUPYTERHUB_USER_LIMIT", "500") }},
...
var settingDescriptions = map[string]string{
	...
	"cache.chat_semantic_scope":          "시맨틱 캐시 재사용 범위: team(기본)·key·global. 이 캐시는 유사도로 맞추므로 global 은 다른 질문에 대한 남의 답이 서빙될 수 있습니다 — 모든 호출자가 한 테넌트일 때만 쓰세요. team/key 에서 신원이 없는 호출자는 캐시를 쓰지 않습니다.",
	...
	"k8s.services.jupyterhub_idle_threshold_minutes": "JupyterHub Named Server 유휴 종료 후보 기준(5~10080분). 서비스별 API 연결 설정에서 재정의할 수 있음.",
	"k8s.services.jupyterhub_http_timeout_seconds":   "JupyterHub REST API 조회·실행 제한 시간(2~120초).",
	"k8s.services.jupyterhub_user_limit":             "한 번의 JupyterHub 사용자/Named Server 조회에서 처리할 최대 사용자 수(1~2000).",
...
func applyRuntimeSetting(t2s *config.Text2SQLConfig, ch *config.ClickHouseConfig, carbon *config.CarbonConfig, ins *config.InsuranceConfig, cache *config.CacheConfig, ret *config.RetentionConfig, pricing *config.PricingConfig, skills *config.SkillsConfig, limits *config.LimitsConfig, logging *config.LoggingConfig, mcp *config.MCPConfig, key, val string) {
	...
	case "cache.chat_semantic_scope":
		cache.ChatSemanticScope = strings.ToLower(strings.TrimSpace(val))
