...
func TestAccessibleMenusByRole(t *testing.T) {
	...
	for _, want := range []string{"k8s-home", "fleet", "k8s", "k8s-pods", "k8s-nodes", "service-catalog", "gitops", "problems", "k8s-rca", "finops", "ai-governance", "enterprise", "settings"} {
		if !aiTabs[want] {
	...
	for _, want := range []string{"k8s-home", "fleet", "k8s-timeline", "gitops", "problems", "k8s-conn", "k8s-actions", "k8s-nodes", "service-catalog", "k8s-meta", "finops", "ai-governance", "k8s-security", "enterprise", "settings"} {
		if !adminTabs[want] {
...
func TestMeNavigationLegacyModeReturnsFullMenu(t *testing.T) {
	...
	for _, want := range []string{"k8s-home", "fleet", "gitops", "problems", "finops", "ai-governance", "enterprise", "settings", "k8s-nodes", "service-catalog", "k8s-security", "runtimesettings"} {
		if !tabs[want] {
