...
func gatewayToolDefs() []mcpToolDef {
	...
	return []mcpToolDef{
		...
		{Name: "k8s_approve_manifest_change", Description: "검증되어 approval_required인 Manifest Change를 승인 원장에 기록합니다. 일반 admin MCP 키에 필요하며 super_admin MCP 키는 apply 시 승인 원장을 자동 기록합니다.", InputSchema: json.RawMessage(`{"type":"object","properties":{"request_id":{"type":"string"},"note":{"type":"string"}},"required":["request_id","note"],"additionalProperties":false}`)},
		{Name: "k8s_apply_manifest_change", Description: "validated 또는 approved Manifest Change를 SSA로 실제 적용합니다. super_admin MCP 키는 승인 없이 직접 실행하되 자동 승인·감사를 기록합니다. admin:write, confirm=true, drift guard와 정책 통과가 필요하며 force drift는 MCP에서 금지합니다.", InputSchema: json.RawMessage(`{"type":"object","properties":{"request_id":{"type":"string"},"confirm":{"type":"boolean","const":true},"note":{"type":"string"}},"required":["request_id","confirm"],"additionalProperties":false}`), Annotations: json.RawMessage(`{"destructiveHint":true,"idempotentHint":false,"openWorldHint":true}`)},
		...
		{Name: "k8s_rollout_precheck", Description: "super_admin MCP API Key로 Deployment·StatefulSet·DaemonSet 또는 Pod 소유 워크로드의 안전 롤아웃 사전검사를 수행합니다. 실제 변경은 없습니다.", InputSchema: json.RawMessage(`{"type":"object","properties":{"cluster_id":{"type":"string"},"namespace":{"type":"string"},"kind":{"type":"string","description":"Deployment|StatefulSet|DaemonSet|Pod"},"name":{"type":"string"}},"required":["cluster_id","namespace","kind","name"],"additionalProperties":false}`)},
		{Name: "k8s_rollout_restart", Description: "super_admin MCP API Key로 안전검사를 통과한 워크로드를 승인 없이 즉시 롤아웃합니다. confirm=true와 사유가 필수이며 감사·이벤트·롤백 원장은 유지됩니다.", InputSchema: json.RawMessage(`{"type":"object","properties":{"cluster_id":{"type":"string"},"namespace":{"type":"string"},"kind":{"type":"string","description":"Deployment|StatefulSet|DaemonSet|Pod"},"name":{"type":"string"},"reason":{"type":"string"},"ticket_no":{"type":"string"},"auto_rollback":{"type":"boolean"},"timeout_seconds":{"type":"integer","minimum":60,"maximum":3600},"confirm":{"type":"boolean","const":true}},"required":["cluster_id","namespace","kind","name","reason","confirm"],"additionalProperties":false}`), Annotations: json.RawMessage(`{"destructiveHint":true,"idempotentHint":false,"openWorldHint":true}`)},
	}
}
...
func (s *Server) runGatewayTool(ctx context.Context, r *http.Request, apiKeyID string, authCtx *store.AuthContext, name string, args json.RawMessage) (map[string]any, error) {
	...
	case "k8s_create_manifest_change", "k8s_validate_manifest_change", "k8s_approve_manifest_change", "k8s_apply_manifest_change", "k8s_verify_manifest_change":
		return s.runGatewayManifestTool(withMCPAdminIdentity(r, apiKeyID, authCtx), authCtx, name, args)
	case "k8s_rollout_precheck", "k8s_rollout_restart":
		return s.runGatewayRolloutTool(r, apiKeyID, authCtx, name, args)
	}
