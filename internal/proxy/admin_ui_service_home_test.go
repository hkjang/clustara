...
func TestServiceHomeOperationsUXContract(t *testing.T) {
	for _, marker := range []string{
		...
		`연결된 Harbor 이미지 사용`,
		`platformAgentLoadHarborProjects`,
		`platformAgentLoadHarborRepositories`,
		`platformAgentLoadHarborArtifacts`,
		`platformAgentUseHarborArtifact`,
		`digest가 있으면 tag보다 digest를 우선합니다`,
		`저장 Credential의 비밀값은 브라우저에 노출되지 않습니다`,
		`배포 이미지`,
