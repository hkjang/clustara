...
var capabilityRegistry = []Capability{
	...
	},
	{
		...
		Description: "클러스터 등록, 리소스 인벤토리, Pod 관리·노드 관리·최근 restart 신호 보정·북마크·최근 이력·로그 분석·실시간 tail·로그 스냅샷·증적 번들·Golden Pod Diff·Health Replay·정책 기반 exec/debug 세션 요청, 터미널 정책, Event/Metric 스냅샷 적재, realtime watch delta 수신, health/security finding, 역할별 개발자 요청, 승인형 액션 요청, Action Flow Navigator, Manifest Change Studio(YAML 요청·검증·승인·drift guard·SSA 적용·증적).",
		...
		UITabs:      []string{"k8s", "k8s-collector", "k8s-pods", "k8s-nodes", "k8s-developer", "k8s-stacks", "k8s-manifest-changes", "k8s-actions", "k8s-settings"},
