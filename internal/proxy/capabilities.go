...
var capabilityRegistry = []Capability{
	...
	},
	{
		...
		Description: "클러스터 등록, 카테고리형 리소스 관리(워크로드·네트워크·스토리지·구성요소·개발자 도구·인증/권한), Pod 관리·노드 관리·최근 restart 신호 보정·북마크·최근 이력·로그 분석·실시간 tail·로그 스냅샷·증적 번들·Golden Pod Diff·Health Replay·정책 기반 exec/debug 세션 요청, 오프라인 xterm.js Full TTY 웹 터미널, 터미널 정책, Event/Metric 스냅샷 적재, realtime watch delta 수신, health/security finding, 역할별 개발자 요청, 승인형 액션 요청, Action Flow Navigator, Manifest Change Studio(YAML 요청·검증·승인·drift guard·SSA 적용·증적).",
