...
var capabilityRegistry = []Capability{
	...
	},
	{
		...
		Description: "클러스터 등록, 리소스 인벤토리, Pod 관리·로그·실시간 tail, Event/Metric 스냅샷 적재, realtime watch delta 수신, health/security finding, 승인형 액션 요청.",
		APIs:        []string{"GET/POST /admin/k8s/clusters", "POST /admin/k8s/snapshot", "POST /admin/k8s/agent/events", "GET /admin/k8s/agent/status", "GET /admin/k8s/pods", "GET /admin/k8s/pods/{namespace}/{pod}/logs", "GET /admin/k8s/pods/{namespace}/{pod}/logs/stream", "GET /admin/k8s/inventory", "GET /admin/k8s/findings", "GET/POST /admin/k8s/actions"},
