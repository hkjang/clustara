...
var apiEndpoints = []apiEndpoint{
	...
	{"/auth/password/change", []string{"post"}, "auth", "Change own password, clear forced-change state, and revoke all sessions", false},
	...
	{"/admin/users/{id}/password-reset", []string{"post"}, "admin", "Reset a user's temporary password, require change, and revoke sessions", false},
	...
	{"/admin/fleet/overview", []string{"get"}, "fleet", "FleetOps overview with cluster group health and rollout risk rollups", false},
	{"/admin/fleet/search", []string{"get"}, "fleet", "Fleet-wide Kubernetes resource search across clusters, groups, owners, labels and specs", false},
	{"/admin/fleet/lifecycle", []string{"get"}, "fleet", "Cluster lifecycle, credential, RBAC and collection freshness status", false},
	{"/admin/fleet/compare", []string{"get"}, "fleet", "Compare inventory/spec/status across two clusters", false},
	{"/admin/fleet/blast-radius", []string{"get"}, "fleet", "Multi-cluster blast radius by image, secret, config, ingress or CVE", false},
	{"/admin/fleet/score", []string{"get"}, "fleet", "Fleet reliability/security/freshness score by cluster", false},
	{"/admin/fleet/actions/dry-run", []string{"post"}, "fleet", "Fleet action dry-run with blockers, warnings and rollback hint", false},
	{"/admin/fleet/progressive-actions", []string{"get", "post"}, "fleet", "Progressive fleet action plans", false},
	{"/admin/security/posture", []string{"get"}, "security", "Enterprise security posture rollup by cluster using inventory, findings and image ledger", false},
	{"/admin/security/access-policy", []string{"get", "post", "put"}, "security", "View, dry-run, or update administrator IP allowlist policy", false},
	{"/admin/security/images", []string{"get"}, "security", "Digest-aware running image ledger and tag drift report", false},
	{"/admin/security/cves", []string{"get", "post"}, "security", "Imported CVE/SBOM vulnerability ledger", false},
	{"/admin/security/signing", []string{"get"}, "security", "Image signing and digest-pinning posture", false},
	{"/admin/security/admission/simulate", []string{"post"}, "security", "Admission policy simulation for manifest changes", false},
	{"/admin/security/runtime-threats", []string{"get", "post"}, "security", "Runtime threat findings and imports", false},
	{"/admin/security/network-graph", []string{"get"}, "security", "Workload, service and ingress network graph", false},
	{"/admin/security/exceptions", []string{"get", "post"}, "security", "Security exception workflow alias", false},
	{"/admin/security/compliance", []string{"get"}, "security", "Compliance report mapped from security findings", false},
	{"/admin/problems", []string{"get"}, "aiops", "AIOps Problem Inbox grouped from K8s incidents to reduce alert noise", false},
	{"/admin/problems/{id}", []string{"get"}, "aiops", "AIOps problem detail with incidents, timeline and graph", false},
	{"/admin/problems/{id}/timeline", []string{"get"}, "aiops", "AIOps incident timeline with events and actions", false},
	{"/admin/problems/{id}/runbook-draft", []string{"get"}, "aiops", "Approval-ready runbook draft for a problem", false},
	{"/admin/problems/{id}/postmortem", []string{"get"}, "aiops", "Postmortem draft from incident evidence", false},
	{"/admin/finops/overview", []string{"get"}, "finops", "FinOps overview combining K8s estimated cost, budgets, anomalies and rightsizing", false},
	{"/admin/finops/costs", []string{"get"}, "finops", "Kubernetes cost allocation by namespace, team, group and cost center", false},
	{"/admin/finops/anomalies", []string{"get"}, "finops", "Kubernetes cost anomaly list from daily namespace snapshots", false},
	{"/admin/finops/rightsizing", []string{"get"}, "finops", "Kubernetes rightsizing recommendations and estimated savings", false},
	{"/admin/finops/billing-imports", []string{"get", "post"}, "finops", "Cloud or CSV billing import ledger", false},
	{"/admin/finops/idle-resources", []string{"get"}, "finops", "Idle deployment, PVC and batch resource candidates", false},
	{"/admin/finops/gpu-costs", []string{"get"}, "finops", "GPU workload cost estimation", false},
	{"/admin/finops/unit-economics", []string{"get"}, "finops", "Cost per request, user or tenant", false},
	{"/admin/finops/chargeback", []string{"get"}, "finops", "Team and cost-center chargeback report", false},
	{"/admin/finops/savings-workflows", []string{"get", "post"}, "finops", "Approval workflows for savings recommendations", false},
	{"/admin/orgs", []string{"get", "post"}, "enterprise", "Enterprise organizations (tenant/governance boundary)", false},
	{"/admin/workspaces", []string{"get", "post"}, "enterprise", "Enterprise workspaces under organizations", false},
	{"/admin/projects", []string{"get", "post"}, "enterprise", "Enterprise projects with owner/environment/cost-center metadata", false},
	{"/admin/catalog/entities", []string{"get", "post"}, "enterprise", "Service/runtime catalog entities for IDP, AIOps and FinOps", false},
	{"/admin/catalog/entities/{id}/runtime", []string{"get"}, "enterprise", "Catalog entity runtime detail combining inventory, incidents, findings and action ledgers", false},
	{"/admin/catalog/coverage", []string{"get"}, "enterprise", "Service catalog runtime linkage coverage by cluster, namespace and kind", false},
	{"/admin/catalog/gap-exceptions", []string{"get", "post"}, "enterprise", "Expiring exceptions for accepted service catalog gaps", false},
	{"/admin/catalog/gaps", []string{"get"}, "enterprise", "Prioritized service catalog gap queue for missing ownership, runtime, docs, project and unlinked resources", false},
	{"/admin/catalog/ownership-map", []string{"get"}, "enterprise", "Ownerless namespace/workload governance map", false},
	{"/admin/catalog/runtime-candidates", []string{"get"}, "enterprise", "Unlinked Kubernetes runtime candidates for service catalog registration", false},
	{"/admin/catalog/scorecards", []string{"get"}, "enterprise", "Service catalog operational scorecards across ownership, runtime, reliability, security and docs", false},
	{"/admin/catalog/self-service-actions", []string{"get", "post"}, "enterprise", "Developer platform self-service action drafts linked to service catalog entities", false},
	{"/admin/access-bindings/evaluate", []string{"post"}, "enterprise", "Evaluate RBAC/ABAC access binding decision for a proposed operation", false},
	{"/admin/access-bindings", []string{"get", "post"}, "enterprise", "RBAC/ABAC access bindings over scoped resources", false},
	{"/admin/enterprise/enforcement", []string{"get"}, "enterprise", "Enterprise enforcement status and guarded route prefixes", false},
	...
	{"/admin/settings/test/k8s-monitoring", []string{"post"}, "settings", "Validate DCGM CSV and test Prometheus GPU telemetry", false},
	...
	{"/admin/ai/governance/overview", []string{"get"}, "ai-governance", "AI Gateway governance posture overview", false},
	{"/admin/gitops/overview", []string{"get"}, "gitops", "GitOps Change Manager overview", false},
	{"/admin/gitops/providers", []string{"get", "post"}, "gitops", "Internal Git provider registry for GitLab and Bitbucket Server", false},
	{"/admin/gitops/providers/{id}", []string{"get", "post", "put", "delete"}, "gitops", "Update or archive a Git provider registry entry", false},
	{"/admin/gitops/providers/test", []string{"post"}, "gitops", "Test GitLab or Bitbucket Server connectivity with a transient token", false},
	{"/admin/gitops/providers/catalog", []string{"post"}, "gitops", "Browse provider projects, repositories, branches, trees, or files", false},
	{"/admin/gitops/providers/pr-template", []string{"post"}, "gitops", "Generate GitLab merge request or Bitbucket Server pull request payload preview", false},
	{"/admin/gitops/sources", []string{"get", "post"}, "gitops", "Git source registry for stacks, services and namespaces", false},
	{"/admin/gitops/drift", []string{"get"}, "gitops", "GitOps drift classification and risk", false},
	{"/admin/gitops/pr-drafts", []string{"get", "post"}, "gitops", "Git PR draft ledger for drift/remediation", false},
	{"/admin/gitops/progressive-rollouts", []string{"get", "post"}, "gitops", "Progressive rollout plans and gates", false},
	{"/admin/gitops/rollback-plans", []string{"get", "post"}, "gitops", "Rollback plan candidates and records", false},
	{"/admin/gitops/change-calendar", []string{"get", "post"}, "gitops", "Freeze and maintenance window calendar", false},
	{"/admin/gitops/deployment-evidence", []string{"get", "post"}, "gitops", "Deployment evidence chain from apply history and records", false},
	{"/admin/governance/evidence", []string{"get", "post"}, "governance", "Evidence chain ledger", false},
	{"/admin/governance/cmdb-links", []string{"get", "post"}, "governance", "CMDB link registry", false},
	{"/admin/governance/tickets", []string{"get", "post"}, "governance", "ITSM ticket registry", false},
	{"/admin/governance/risk-register", []string{"get", "post"}, "governance", "Accepted risk register", false},
	{"/admin/governance/executive-report", []string{"get"}, "governance", "Executive operations report across stability, security, cost and risk", false},
	{"/admin/governance/audit-search", []string{"get"}, "governance", "Unified audit search across admin, auth and enterprise records", false},
	...
	{"/ingest/k8s/agent/events", []string{"post"}, "k8s", "Ingest realtime Kubernetes watch delta events with a cluster-scoped agent token", false},
	...
	{"/admin/k8s/build/analyze", []string{"post"}, "k8s", "Analyze build metadata and Kubernetes deployment evidence", false},
	{"/admin/k8s/build-definitions", []string{"get", "post"}, "k8s", "List/create Kubernetes build definitions", false},
	{"/admin/k8s/build-runs", []string{"get", "post"}, "k8s", "List/create Kubernetes build run records", false},
	{"/admin/k8s/capacity", []string{"get"}, "k8s", "Kubernetes capacity and autoscaling analysis", false},
	{"/admin/k8s/capacity/simulate", []string{"post"}, "k8s", "Simulate capacity impact for replica or resource changes", false},
	{"/admin/k8s/collect-bursts", []string{"get", "post"}, "k8s", "List/request change-aware high-frequency collection bursts", false},
	{"/admin/k8s/collect-config", []string{"get", "post"}, "k8s", "Get/update Kubernetes collection scheduler configuration", false},
	{"/admin/k8s/collect-slo", []string{"get"}, "k8s", "Kubernetes collection SLO and freshness analysis", false},
	{"/admin/k8s/collection-cost", []string{"get"}, "k8s", "Estimate collection workload and API cost", false},
	{"/admin/k8s/config-changes", []string{"get", "post"}, "k8s", "List/create ConfigMap or Secret change requests", false},
	{"/admin/k8s/config-changes/{id}", []string{"get", "post"}, "k8s", "Operate one ConfigMap or Secret change request", false},
	{"/admin/k8s/config-impact", []string{"get"}, "k8s", "Analyze ConfigMap or Secret blast radius", false},
	{"/admin/k8s/connectivity", []string{"get"}, "k8s", "Kubernetes service, ingress, endpoint, PVC, rollout, and job connectivity checks", false},
	{"/admin/k8s/cost", []string{"get"}, "k8s", "Kubernetes request-based cost estimate", false},
	{"/admin/k8s/cost/config", []string{"get", "post"}, "k8s", "Get/update Kubernetes cost model configuration", false},
	{"/admin/k8s/cost/recommendations", []string{"get"}, "k8s", "Kubernetes cost optimization recommendations", false},
	{"/admin/k8s/cost/snapshot", []string{"post"}, "k8s", "Persist a Kubernetes cost snapshot", false},
	{"/admin/k8s/cost/trend", []string{"get"}, "k8s", "Kubernetes cost trend analysis", false},
	{"/admin/k8s/dev-requests", []string{"get", "post"}, "k8s", "Developer-facing Kubernetes operation requests", false},
	{"/admin/k8s/diff", []string{"get"}, "k8s", "Field-level Kubernetes resource diff", false},
	{"/admin/k8s/discovery", []string{"get"}, "k8s", "Kubernetes API discovery and schema catalog", false},
	{"/admin/k8s/discovery/activate", []string{"post"}, "k8s", "Activate discovered Kubernetes resource collection candidates", false},
	{"/admin/k8s/discovery/compare", []string{"get"}, "k8s", "Compare discovery results across clusters or time", false},
	{"/admin/k8s/dw/bootstrap", []string{"post"}, "k8s", "Create Kubernetes ClickHouse fact tables", false},
	{"/admin/k8s/dw/report", []string{"get"}, "k8s", "Query Kubernetes long-term analytics report", false},
	{"/admin/k8s/dw/sink", []string{"post"}, "k8s", "Ship current Kubernetes facts to ClickHouse", false},
	{"/admin/k8s/exposures", []string{"get"}, "k8s", "External exposure center for Ingress and exposed Services", false},
	{"/admin/k8s/extensions/install-plan", []string{"get"}, "k8s", "Kubernetes extension install plan and readiness", false},
	...
	{"/admin/k8s/images", []string{"get"}, "k8s", "Kubernetes workload image inventory and tag policy findings", false},
	{"/admin/k8s/incidents", []string{"get"}, "k8s", "List Kubernetes incidents", false},
	{"/admin/k8s/incidents/{id}", []string{"get", "post"}, "k8s", "Get or update one Kubernetes incident", false},
	{"/admin/k8s/latency/collect", []string{"post"}, "k8s", "Collect Kubernetes latency samples", false},
	{"/admin/k8s/latency/config", []string{"get", "post"}, "k8s", "Get/update Kubernetes latency collection config", false},
	{"/admin/k8s/lifecycle", []string{"get"}, "k8s", "Kubernetes cluster lifecycle, credential, and collection readiness", false},
	{"/admin/k8s/manifest", []string{"get"}, "k8s", "Read a masked live Kubernetes resource manifest", false},
	{"/admin/k8s/manifest-changes", []string{"get", "post"}, "k8s", "List/create Manifest Change Studio update/create requests", false},
	{"/admin/k8s/manifest-changes/{id}", []string{"get", "post"}, "k8s", "Validate, approve, apply, verify, rollback, or export one manifest create/update request", false},
	{"/admin/k8s/manifests/editor", []string{"get"}, "k8s", "Manifest Change Studio editor metadata and resource picker data", false},
	{"/admin/k8s/manifests/live", []string{"get"}, "k8s", "Load a live Kubernetes manifest for editing", false},
	{"/admin/k8s/node-drain", []string{"get"}, "k8s", "Node drain impact and safety analysis", false},
	{"/admin/k8s/notify/config", []string{"get", "post"}, "k8s", "Get/update Kubernetes notification configuration", false},
	...
	{"/admin/k8s/nodes/monitoring", []string{"get"}, "k8s", "Node CPU, memory, GPU and explainable failure-risk monitoring", false},
	{"/admin/k8s/node-metrics/collect", []string{"post"}, "k8s", "Trigger a lightweight node metrics and optional DCGM refresh", false},
	{"/admin/k8s/gpu/operations", []string{"get"}, "k8s", "GPU device, MIG, workload, waste, VRAM, hardware and cost operations", false},
	{"/admin/k8s/gpu/policy", []string{"get", "post"}, "k8s", "GPU alert thresholds and hourly cost policy", false},
	{"/admin/k8s/gpu/dcgm-config", []string{"get"}, "k8s", "Effective DCGM counters CSV validation and ConfigMap preview", false},
	...
	{"/admin/k8s/policies/{id}", []string{"get", "post", "delete"}, "k8s", "Get/update/delete one Kubernetes policy", false},
	{"/admin/k8s/policies/compliance", []string{"get"}, "k8s", "Kubernetes policy compliance report", false},
	{"/admin/k8s/policies/export", []string{"get"}, "k8s", "Export Kubernetes policy packs", false},
	{"/admin/k8s/policies/import", []string{"post"}, "k8s", "Import Kubernetes policy packs", false},
	{"/admin/k8s/policies/simulate", []string{"post"}, "k8s", "Simulate Kubernetes policy decisions for a manifest", false},
	{"/admin/k8s/rbac", []string{"get"}, "k8s", "Kubernetes RBAC risk analysis", false},
	{"/admin/k8s/rbac-diff", []string{"get"}, "k8s", "Kubernetes RBAC revision diff", false},
	{"/admin/k8s/rbac/check", []string{"post"}, "k8s", "Check whether a Kubernetes subject can perform an action", false},
	{"/admin/k8s/registries/pull-secret", []string{"post"}, "k8s", "Generate or validate registry pull secret guidance", false},
	{"/admin/harbor/registries", []string{"get", "post"}, "k8s", "List/register Harbor registry instances for image launch flows", false},
	{"/admin/harbor/registries/{id}", []string{"get", "post", "delete"}, "k8s", "Get/update/delete one Harbor registry metadata row", false},
	{"/admin/harbor/registries/{id}/test", []string{"post"}, "k8s", "Test Harbor /api/v2.0/systeminfo connectivity", false},
	{"/admin/harbor/robots", []string{"get", "post"}, "k8s", "List/register Harbor Robot Accounts without returning tokens", false},
	{"/admin/harbor/robots/{id}", []string{"get", "post", "delete"}, "k8s", "Get/update/delete one Harbor Robot Account metadata row without returning token material", false},
	{"/admin/harbor/robots/verify", []string{"post"}, "k8s", "Verify one-time Harbor Robot pull permission", false},
	{"/admin/harbor/mappings", []string{"get", "post"}, "k8s", "Map Harbor projects to Kubernetes cluster namespaces and pull secrets", false},
	{"/admin/harbor/mappings/{id}", []string{"get", "post", "delete"}, "k8s", "Get/update/delete one Harbor project namespace mapping", false},
	{"/admin/harbor/catalog/query", []string{"post"}, "k8s", "Query Harbor projects, repositories, artifacts, tags and digests with optional one-time robot token", false},
	{"/admin/harbor/pull-secret/preview", []string{"post"}, "k8s", "Generate redacted imagePullSecret manifest preview for a Harbor robot account", false},
	{"/admin/harbor/launches/preview", []string{"post"}, "k8s", "Preview digest-first Deployment/Service manifests and launch policy findings", false},
	{"/admin/harbor/launches", []string{"get", "post"}, "k8s", "List/create Harbor application launch ledger requests", false},
	{"/admin/harbor/launches/{id}/manifest-change", []string{"post"}, "k8s", "Create Manifest Change Studio Deployment/Service drafts from one Harbor launch request", false},
	{"/admin/k8s/remediation/advice", []string{"get"}, "k8s", "Kubernetes remediation advisor recommendations", false},
	{"/admin/k8s/report-schedules", []string{"get", "post"}, "k8s", "List/create Kubernetes report schedules", false},
	{"/admin/k8s/report-schedules/{id}", []string{"get", "post", "delete"}, "k8s", "Get/update/delete one Kubernetes report schedule", false},
	{"/admin/k8s/reports", []string{"get", "post"}, "k8s", "Generate or list Kubernetes operations reports", false},
	{"/admin/k8s/resource-advisor", []string{"get"}, "k8s", "Kubernetes resource request and limit advisor", false},
	{"/admin/k8s/revisions", []string{"get"}, "k8s", "Kubernetes resource revision history", false},
	{"/admin/k8s/runtime-security", []string{"get"}, "k8s", "Runtime security profile for Pods and workloads", false},
	{"/admin/k8s/security", []string{"get"}, "k8s", "Kubernetes security posture and findings", false},
	{"/admin/k8s/security/vuln/summary", []string{"get"}, "k8s", "Image vulnerability rollup by severity, exceptions, runtime and CIS signals", false},
	{"/admin/k8s/security/vuln/images", []string{"get"}, "k8s", "List normalized Trivy/Grype image vulnerabilities by digest, image, workload and severity", false},
	{"/admin/k8s/security/vuln/workloads", []string{"get"}, "k8s", "Workload impact view for vulnerable running images", false},
	...
	{"/admin/k8s/security/exceptions/{id}/revoke", []string{"post"}, "k8s", "Revoke a security exception", false},
	{"/admin/k8s/security/admission/evaluate", []string{"post"}, "k8s", "Evaluate manifest or images against digest, SBOM and vulnerability policy", false},
	{"/admin/k8s/security/admission/review", []string{"post"}, "k8s", "AdmissionReview-compatible image security decision endpoint", false},
	{"/admin/k8s/security/admission/decisions", []string{"get"}, "k8s", "List admission allow, warning, approval and deny decisions", false},
	{"/admin/k8s/security/runtime/events", []string{"get", "post"}, "k8s", "Ingest and list Falco/Falcosidekick runtime events", false},
	{"/admin/k8s/security/benchmarks/job-manifest", []string{"get", "post"}, "k8s", "Generate a kube-bench Kubernetes Job manifest without executing it", false},
	{"/admin/k8s/security/benchmarks/runs", []string{"get", "post"}, "k8s", "Import and list kube-bench CIS benchmark runs", false},
	{"/admin/k8s/security/benchmarks/results", []string{"get"}, "k8s", "List kube-bench CIS benchmark results for a run", false},
	{"/admin/k8s/security-exceptions", []string{"get", "post"}, "k8s", "List/create Kubernetes security policy exceptions", false},
	{"/admin/k8s/security-exceptions/{id}", []string{"get", "post", "delete"}, "k8s", "Get/update/delete one Kubernetes security exception", false},
	{"/admin/k8s/service-impact", []string{"get"}, "k8s", "Kubernetes service impact and blast-radius analysis", false},
	{"/admin/k8s/slo", []string{"get"}, "k8s", "Kubernetes incident-based SLO and error-budget report", false},
	{"/admin/k8s/stacks", []string{"get", "post"}, "k8s", "List/create Application Stack definitions", false},
	{"/admin/k8s/stacks/{id}", []string{"get", "post"}, "k8s", "Operate one Application Stack revision, apply, promote, rollback, or history request", false},
	{"/admin/k8s/stacks/validate", []string{"post"}, "k8s", "Validate an Application Stack manifest set", false},
	{"/admin/k8s/services/catalogs", []string{"get", "post"}, "service-platform", "List or register deployable service catalog definitions", false},
	{"/admin/k8s/services/catalogs/{id}", []string{"get", "put", "delete"}, "service-platform", "Get, update, or disable a service catalog", false},
	{"/admin/k8s/services/catalogs/{id}/versions", []string{"post"}, "service-platform", "Register an append-only service template version", false},
	{"/admin/k8s/services/catalogs/{id}/validate", []string{"post"}, "service-platform", "Render and policy-check a service specification", false},
	...
	{"/admin/k8s/services/instances/{id}/restart", []string{"post"}, "service-platform", "Create an Action Center rollout restart request", false},
	{"/admin/k8s/services/instances/{id}/scale", []string{"post"}, "service-platform", "Create an Action Center scale request", false},
	...
	{"/admin/k8s/services/instances/{id}/backups", []string{"get", "post"}, "service-platform", "List backup ledger or create an approval-ready PostgreSQL SQL, Redis RDB, Jupyter workspace, or CSI VolumeSnapshot backup draft", false},
	{"/admin/k8s/services/instances/{id}/jupyter-workspaces", []string{"get"}, "service-platform", "Map JupyterHub users to notebook Pods and PVCs using deployment labels and volume mounts", false},
	{"/admin/k8s/services/instances/{id}/jupyter-config", []string{"get", "post"}, "service-platform", "Manage and test a service-scoped encrypted JupyterHub API credential and idle policy", false},
	{"/admin/k8s/services/instances/{id}/jupyter-servers", []string{"get"}, "service-platform", "List JupyterHub default and named servers with activity-based idle candidates", false},
	{"/admin/k8s/services/instances/{id}/jupyter-server-actions", []string{"post"}, "service-platform", "Create approval-gated JupyterHub Named Server start and stop requests", false},
	{"/admin/k8s/services/instances/{id}/jupyter-idle-policy", []string{"post"}, "service-platform", "Preview idle JupyterHub servers or queue idempotent Action Center stop approvals", false},
	{"/admin/k8s/services/backups/{backupId}/restore-preview", []string{"post"}, "service-platform", "Validate PostgreSQL/Redis/JupyterLab restore or safe CSI VolumeSnapshot clone-PVC impact before approval", false},
	{"/admin/k8s/services/backups/{backupId}/restore", []string{"post"}, "service-platform", "Create an approval-ready database/workspace restore Job or VolumeSnapshot clone-PVC draft", false},
	{"/admin/k8s/services/reconcile", []string{"get", "post"}, "service-platform", "Inspect, dry-run, or trigger the periodic ServiceInstance reconciliation worker", false},
	{"/admin/k8s/timeline", []string{"get"}, "k8s", "Kubernetes change, event, revision, action, and manifest timeline", false},
	{"/admin/k8s/workspace-template", []string{"get"}, "k8s", "Generate a Kubernetes workspace template", false},
	{"/admin/k8s/workspaces", []string{"get", "post"}, "k8s", "List/create Kubernetes developer workspaces", false},
	{"/admin/k8s/pods", []string{"get"}, "k8s", "Pod management center: list Pods with status, owner, node, cumulative restarts, recent restart signal, and risk", false},
	{"/admin/k8s/pods/{namespace}/{pod}", []string{"get"}, "k8s", "Pod detail: status, containers, startedAt-based restart recency, events, metrics, masked manifest", false},
	...
	{"/admin/k8s/pods/{namespace}/{pod}/lifecycle", []string{"get"}, "k8s", "Read the durable UID-based Pod and container lifecycle ledger, including deleted Pods", false},
	...
	{"/admin/k8s/action-flow", []string{"get"}, "k8s", "Operator action-flow navigator across actions, config/YAML changes, exec, and debug sessions", false},
	...
	{"/admin/mcp/tool-scopes", []string{"get", "post"}, "mcp", "MCP tool ACL and masking scope rules", false},
	{"/admin/mcp/tool-scopes/{id}", []string{"get", "post", "delete"}, "mcp", "Get, update, or delete one MCP tool scope rule", false},
	...
	{"/admin/agent/action-cards", []string{"get"}, "agent", "Agent action card queue", false},
	{"/admin/agent/action-cards/{id}", []string{"get", "post"}, "agent", "Get or update one agent action card", false},
	...
	{"/admin/agent/sessions/{id}", []string{"get", "post"}, "agent", "Get or update one agent session", false},
	{"/admin/agent/suggestions", []string{"get", "post"}, "agent", "Agent suggestions and operator feedback", false},
	...
	{"/admin/ops/status", []string{"get"}, "admin", "Ops health status with readiness v2 components", false},
	...
	{"/integrations/mattermost/command", []string{"post"}, "notifications", "Mattermost slash-command inbound endpoint (token-verified)", true},
}
