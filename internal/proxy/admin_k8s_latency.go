...
// Prometheus credentials, PromQL and label mapping come from runtime settings with env fallback.
...
func (s *Server) handleK8sLatencyCollect(w http.ResponseWriter, r *http.Request) {
	...
	promURL, promToken := s.monitoringPrometheusConfig(r.Context())
	if promURL == "" {
		writeJSON(w, http.StatusOK, map[string]any{"collected": false, "note": "Prometheus가 구성되지 않았습니다 (런타임 설정 k8s.monitoring.prometheus_url)."})
		return
	}
	promQL := s.runtimeSettingValue(r.Context(), "k8s.monitoring.latency_promql")
	...
	nsLabel := firstNonEmpty(s.runtimeSettingValue(r.Context(), "k8s.monitoring.latency_namespace_label"), "namespace")
	nameLabel := firstNonEmpty(s.runtimeSettingValue(r.Context(), "k8s.monitoring.latency_name_label"), "workload")
...
func (s *Server) handleK8sLatencyConfig(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		writeJSON(w, http.StatusOK, map[string]any{
			...
			"promql":         s.runtimeSettingValue(r.Context(), "k8s.monitoring.latency_promql"),
			"ns_label":       firstNonEmpty(s.runtimeSettingValue(r.Context(), "k8s.monitoring.latency_namespace_label"), "namespace"),
			"name_label":     firstNonEmpty(s.runtimeSettingValue(r.Context(), "k8s.monitoring.latency_name_label"), "workload"),
		})
	case http.MethodPost:
		...
		set := func(key string, v *string) error {
			...
			return s.persistSettingValue(r, d, strings.TrimSpace(*v), "Kubernetes latency config")
		}
		if err := set("k8s.monitoring.latency_promql", p.PromQL); err != nil {
			writeOpenAIError(w, http.StatusInternalServerError, err.Error(), "server_error", "flag_save_failed")
			return
		}
		if err := set("k8s.monitoring.latency_namespace_label", p.NSLabel); err != nil {
			writeOpenAIError(w, http.StatusBadRequest, err.Error(), "invalid_request_error", "setting_invalid")
			return
		}
		if err := set("k8s.monitoring.latency_name_label", p.NameLabel); err != nil {
			writeOpenAIError(w, http.StatusBadRequest, err.Error(), "invalid_request_error", "setting_invalid")
			return
		}
