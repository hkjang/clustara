...
<body>
  ...
  <script>
    ...
    async function bootAfterAuth(me) {
      ...
      const home = (authState.nav && authState.nav.default_home) || '#/k8s-home';
      if (atRoot && home !== '#/k8s-home') {
    ...
    function renderMarkdown(md) {
      ...
      // 7. Blockquotes
      html = html.replace(/^\>\s+(.*?)$/gm, '<blockquote style="border-left:4px solid var(--accent, #1d4ed8); padding-left:12px; color:var(--muted); margin:8px 0; font-style:italic;">$1</blockquote>');
    ...
    function parseHash() {
      const raw = (location.hash || '#/k8s-home').replace(/^#\//, '');
    ...
    // Agent Session Flight Recorder — 한 세션의 Clustara 활동을 시간순으로 재구성.
    ...
    function vcsEventsHTML(events) {
      if (!events.length) {
        return '<h4 style="margin:16px 0 6px; font-size:14px">연결된 VCS (커밋 / MR)</h4>' +
          '<div class="muted" style="font-size:12px">이 세션에 연결된 커밋·MR 이 없습니다. 커밋 메시지/브랜치/ MR 제목에 <code>Vibe-Session: ' + '…' + '</code> 마커를 넣고 GitLab/Bitbucket 웹훅(또는 CI)을 Clustara로 보내면 Prompt→Commit→MR 이 연결됩니다.</div>';
      }
      const kindBadge = (e) => e.kind === 'merge_request'
        ? '<span class="status ' + (e.state === 'merged' ? '' : (e.state === 'closed' ? 'error' : 'warn')) + '">MR ' + escapeHTML(e.state || '') + '</span>'
        : '<span class="pill">' + escapeHTML(e.kind || 'commit') + '</span>' + (e.provider === 'inferred' ? ' <span class="muted" style="font-size:11px">추론</span>' : '');
    ...
    async function renderUserDetail(id) {
      ...
      if (isExternal) {
        ...
        notice = '<div class="banner error" style="margin:0 14px 12px"><strong>이 키로 집계된 요청이 0건입니다.</strong> 클라이언트가 <em>이 키를 정확히</em> 보내고 있는지 확인하세요(키 오타·공백·옛 키·잘못된 Bearer 값). ' +
          '실제 트래픽은 <a href="#/users">사용자 목록</a>의 <code>passthrough</code>/<code>anonymous</code> 또는 상태가 <code>external</code> 인 항목에 있을 수 있습니다. 클라이언트가 보내는 키가 곧 사용자 식별자이며, 등록된 키와 글자 단위로 일치해야 집계됩니다. 응답 헤더 <code>X-Api-Key-Id</code> 로 Clustara가 어떤 식별자로 인식했는지 확인할 수 있습니다.</div>';
      }
      const html =
    ...
    async function renderK8sHome() {
      ...
      const riskRows = (d.clusters_at_risk || []).length ? (d.clusters_at_risk || []).map(c =>
        ...
        '<td>' + escapeHTML(c.status || '-') + '</td><td>' + fmt(c.score) + '</td></tr>').join('')
        : '<tr><td colspan="3" class="muted">위험 클러스터 없음.</td></tr>';

      const failRows = (d.failure_candidates || []).length ? (d.failure_candidates || []).map(f =>
      ...
      const changeRows = (d.recent_changes || []).length ? (d.recent_changes || []).map(c =>
        '<tr><td class="muted" style="font-size:11px">' + ago(c.observed_at) + '</td>' +
        '<td>' + escapeHTML((c.cluster_name || '') + ' · ' + (c.namespace || '-') + '/' + c.kind + '/' + c.name) + '</td>' +
        '<td class="muted" style="font-size:11px">' + escapeHTML(c.image_set || 'spec 변경') + '</td>' +
        '<td><a href="' + escapeAttr(tlHref(c)) + '">Diff</a></td></tr>').join('')
    ...
    function k8sRegisterGuideHTML() {
      return '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px">' +
        ...
        '<h3 style="font-size:14px;margin:0 0 8px">개발 PC · minikube</h3>' +
        '<ol style="margin:0;padding-left:18px;font-size:12px;line-height:1.65">' +
        '<li><code>minikube status</code>로 실행 상태를 확인합니다.</li>' +
        '<li><code>kubectl config use-context minikube</code>로 현재 context를 맞춥니다.</li>' +
        '<li><code>kubectl config view --minify -o jsonpath={.clusters[0].cluster.server}</code> 값을 API Server URL에 넣습니다.</li>' +
        '<li><code>kubectl config view --raw --minify --flatten</code> 출력 전체를 kubeconfig 입력칸에 붙여 넣습니다.</li>' +
        '<li>이름은 <code>local-minikube</code>, 인증 방식은 <code>kubeconfig</code>로 등록한 뒤 연결 테스트를 누릅니다.</li>' +
        '</ol>' +
        '<p class="muted" style="font-size:11px;margin:8px 0 0">TLS unknown authority 오류가 나면 인증서가 포함되지 않은 kubeconfig일 가능성이 큽니다. <code>--flatten</code>으로 다시 출력해 등록하세요.</p>' +
        '<p class="muted" style="font-size:11px;margin:4px 0 0">게이트웨이를 Docker 안에서 실행하면 minikube API 주소가 컨테이너에서 닿지 않을 수 있습니다. 이 경우 host 접근 주소를 별도로 맞춰야 합니다.</p>' +
        '</div>' +
        '<div style="border:1px solid var(--border);border-radius:8px;padding:12px">' +
        '<h3 style="font-size:14px;margin:0 0 8px">운영망 · K8s cluster</h3>' +
        '<ol style="margin:0;padding-left:18px;font-size:12px;line-height:1.65">' +
        '<li>전용 ServiceAccount를 만들고 get/list/watch 중심의 최소 권한 ClusterRole을 바인딩합니다.</li>' +
        '<li><code>kubectl -n clustara-system create token clustara-reader --duration=8760h</code>로 토큰을 발급합니다.</li>' +
        '<li>사설 CA를 쓰면 token만 넣기보다 CA가 포함된 전용 kubeconfig를 만들어 <code>kubeconfig</code> 방식으로 등록합니다.</li>' +
        '<li>운영 클러스터 이름은 환경과 리전을 드러내도록 <code>prod-kr-a</code>처럼 정합니다.</li>' +
        '<li>등록 직후 연결 테스트와 수집을 차례로 눌러 버전, 노드 수, 네임스페이스 수가 갱신되는지 확인합니다.</li>' +
        '</ol>' +
        '<p class="muted" style="font-size:11px;margin:8px 0 0">delete pod, drain, manifest 적용 같은 위험 액션은 별도 승인 흐름으로 다루는 권한을 분리하는 구성이 좋습니다.</p>' +
        '</div>' +
        '<div style="border:1px solid var(--border);border-radius:8px;padding:12px">' +
        '<h3 style="font-size:14px;margin:0 0 8px">공통 체크</h3>' +
        '<table><tbody>' +
        '<tr><th>API Server URL</th><td><code>https://...</code> 형태의 Kubernetes API endpoint</td></tr>' +
        '<tr><th>인증 방식</th><td>minikube는 kubeconfig, 운영망은 CA 포함 kubeconfig 또는 전용 token 권장</td></tr>' +
        '<tr><th>네트워크</th><td>Clustara가 실행되는 서버에서 API Server 443/TCP에 도달 가능해야 합니다.</td></tr>' +
        '<tr><th>등록 후</th><td>연결 테스트 → 수집 → 인벤토리와 장애 분석 결과 확인 순서로 검증합니다.</td></tr>' +
        '</tbody></table>' +
        '</div>' +
        '</div>';
    }

    async function renderK8sOperations() {
      const view = document.getElementById('view');
      view.innerHTML = section('K8s 운영', '<div class="empty">불러오는 중...</div>');
      let overview, clusters, inventory, events, findings, rca, actions;
      try {
        [overview, clusters, inventory, events, findings, rca, actions] = await Promise.all([
          api('/admin/k8s/overview'),
          api('/admin/k8s/clusters'),
          api('/admin/k8s/inventory?limit=25'),
          api('/admin/k8s/events?limit=20'),
          api('/admin/k8s/findings?limit=20'),
          api('/admin/k8s/rca'),
          api('/admin/k8s/actions?limit=20'),
        ]);
      } catch (e) {
        view.innerHTML = section('K8s 운영', '<div class="card-body" style="padding:16px"><p class="muted">' + escapeHTML(e.message) + '</p></div>');
        return;
      }
      const c = overview.clusters || {};
      const inv = overview.inventory || [];
      const statusBadge = (st) => st === 'ready' || st === 'connected' ? '<span class="status">정상</span>' : (st === 'error' ? '<span class="status error">위험</span>' : '<span class="status warn">' + escapeHTML(st || '미상') + '</span>');
      const clusterRows = (clusters.clusters || []).length ? (clusters.clusters || []).map(cl =>
        '<tr><td><strong>' + escapeHTML(cl.name) + '</strong><div class="muted" style="font-size:11px">' + escapeHTML(cl.id) + '</div></td>' +
        '<td>' + statusBadge(cl.status) + '</td><td>' + escapeHTML(cl.kubernetes_version || '-') + '</td>' +
        '<td>' + fmt(cl.node_count) + '</td><td>' + fmt(cl.namespace_count) + '</td>' +
      ...
      const invRows = (inventory.items || []).length ? (inventory.items || []).map(it =>
        '<tr><td>' + escapeHTML(it.kind) + '</td><td>' + escapeHTML(it.namespace || '-') + '</td><td>' + escapeHTML(it.name) + '</td>' +
        '<td>' + escapeHTML(it.status || '-') + '</td><td>' + fmt(it.health_score) + '</td><td>' + escapeHTML(it.risk_level || '-') + '</td>' +
        '<td><a href="' + escapeAttr(timelineHref(it)) + '">이력·Diff</a></td></tr>'
      ).join('') : '<tr><td colspan="7" class="muted">수집된 리소스가 없습니다.</td></tr>';
      const findingRows = (findings.findings || []).length ? (findings.findings || []).map(f =>
        '<tr><td><span class="status ' + (f.severity === 'critical' || f.severity === 'high' ? 'error' : (f.severity === 'medium' ? 'warn' : '')) + '">' + escapeHTML(f.severity) + '</span></td>' +
        '<td>' + escapeHTML(f.rule) + '</td><td>' + escapeHTML(f.namespace || '-') + '</td><td>' + escapeHTML(f.resource_kind + '/' + f.resource_name) + '</td>' +
        '<td class="muted" style="font-size:11px">' + escapeHTML(f.message) + '</td></tr>'
      ).join('') : '<tr><td colspan="5" class="muted">열린 Finding이 없습니다.</td></tr>';
      const actionRows = (actions.actions || []).length ? (actions.actions || []).map(a =>
        '<tr><td>' + escapeHTML(a.action) + '</td><td>' + escapeHTML(a.namespace || '-') + '</td><td>' + escapeHTML(a.resource_kind + '/' + a.resource_name) + '</td>' +
        '<td><span class="status ' + (a.risk_level === 'critical' || a.risk_level === 'high' ? 'error' : 'warn') + '">' + escapeHTML(a.risk_level) + '</span></td>' +
        '<td>' + escapeHTML(a.status) + '</td><td class="muted" style="font-size:11px">' + escapeHTML(a.result || '') + '</td></tr>'
      ).join('') : '<tr><td colspan="6" class="muted">액션 요청이 없습니다.</td></tr>';
      const eventRows = (events.events || []).length ? (events.events || []).map(e =>
        '<tr><td>' + escapeHTML(e.type || '-') + '</td><td>' + escapeHTML(e.reason || '-') + '</td><td>' + escapeHTML(e.namespace || '-') + '</td>' +
        '<td>' + escapeHTML(e.involved_kind + '/' + e.involved_name) + '</td><td class="muted" style="font-size:11px">' + escapeHTML(e.message || '') + '</td></tr>'
      ).join('') : '<tr><td colspan="5" class="muted">이벤트가 없습니다.</td></tr>';
      const rcaRows = (rca.candidates || []).length ? (rca.candidates || []).map(x =>
        '<tr><td><span class="status ' + (x.severity === 'critical' || x.severity === 'high' ? 'error' : 'warn') + '">' + escapeHTML(x.severity) + '</span></td>' +
        '<td>' + escapeHTML(x.condition) + '</td><td>' + escapeHTML((x.namespace || '-') + ' / ' + x.resource_kind + '/' + x.resource_name) + '</td>' +
        '<td class="muted" style="font-size:11px">' + escapeHTML(x.cause || '') + '</td>' +
        '<td class="muted" style="font-size:11px">' + escapeHTML((x.actions || []).slice(0, 2).join(' · ')) + '</td></tr>'
      ).join('') : '<tr><td colspan="5" class="muted">장애 분석 후보가 없습니다.</td></tr>';
      view.innerHTML =
        section('K8s 운영',
          '<div class="kpis">' +
          kpi('클러스터', fmt(c.total || 0) + ' / 정상 ' + fmt(c.ready || 0)) +
          kpi('위험 클러스터', fmt(c.risky || 0)) +
          kpi('Warning 24h', fmt(overview.warning_events_24h || 0)) +
          kpi('열린 Finding', fmt(overview.open_findings || 0)) +
          kpi('대기 액션', fmt(overview.pending_actions || 0)) +
          '</div>') +
        card('클러스터 등록',
          '<div class="card-body"><div style="display:flex;gap:6px;flex-wrap:wrap">' +
          '<input id="k8s-id" type="hidden">' +
          '<input id="k8s-name" placeholder="클러스터 이름" style="min-width:160px">' +
          '<input id="k8s-url" placeholder="API Server URL" style="min-width:260px">' +
          '<select id="k8s-auth"><option value="kubeconfig">kubeconfig</option><option value="token">token</option><option value="service_account">service_account</option><option value="in_cluster">in_cluster</option></select>' +
          '<button id="k8s-submit" type="button" onclick="k8sRegister()">등록</button><button type="button" class="secondary" onclick="k8sRegisterGuide()">등록 가이드</button><button type="button" class="secondary" onclick="k8sResetForm()">초기화</button></div>' +
          '<textarea id="k8s-credential" rows="4" placeholder="kubeconfig 또는 token" style="width:100%;margin-top:6px"></textarea><div id="k8s-msg" class="muted" style="font-size:12px;margin-top:4px"></div></div>') +
        card('클러스터 목록',
          '<div class="card-body"><table><thead><tr><th>클러스터</th><th>상태</th><th>버전</th><th>노드</th><th>네임스페이스</th><th>마지막 상태</th><th></th></tr></thead><tbody>' + clusterRows + '</tbody></table></div>') +
        card('리소스 인벤토리',
          '<div class="card-body"><div class="muted" style="font-size:12px;margin-bottom:6px">' + inv.map(x => escapeHTML(x.kind) + ' ' + fmt(x.count)).join(' · ') + '</div><table><thead><tr><th>Kind</th><th>Namespace</th><th>Name</th><th>Status</th><th>Health</th><th>Risk</th><th></th></tr></thead><tbody>' + invRows + '</tbody></table></div>') +
        card('장애·보안 Finding',
          '<div class="card-body"><table><thead><tr><th>심각도</th><th>규칙</th><th>Namespace</th><th>리소스</th><th>메시지</th></tr></thead><tbody>' + findingRows + '</tbody></table></div>') +
        card('장애 분석',
          '<div class="card-body"><table><thead><tr><th>심각도</th><th>상태</th><th>대상</th><th>원인 후보</th><th>조치 후보</th></tr></thead><tbody>' + rcaRows + '</tbody></table></div>') +
        card('이벤트 타임라인',
          '<div class="card-body"><table><thead><tr><th>Type</th><th>Reason</th><th>Namespace</th><th>대상</th><th>Message</th></tr></thead><tbody>' + eventRows + '</tbody></table></div>') +
        card('액션 센터',
          '<div class="card-body"><table><thead><tr><th>Action</th><th>Namespace</th><th>대상</th><th>Risk</th><th>Status</th><th>결과</th></tr></thead><tbody>' + actionRows + '</tbody></table></div>');
    }
    window.k8sRegisterGuide = () => {
      openModal('클러스터 등록 가이드', k8sRegisterGuideHTML(), null, { wide: true });
    };
    window.k8sResetForm = () => {
      const id = document.getElementById('k8s-id');
      const name = document.getElementById('k8s-name');
      const url = document.getElementById('k8s-url');
      const auth = document.getElementById('k8s-auth');
      const credential = document.getElementById('k8s-credential');
      const submit = document.getElementById('k8s-submit');
      const msg = document.getElementById('k8s-msg');
      if (id) id.value = '';
      if (name) name.value = '';
      if (url) url.value = '';
      if (auth) auth.value = 'kubeconfig';
      if (credential) credential.value = '';
      if (submit) submit.textContent = '등록';
      if (msg) msg.textContent = '';
    };
    window.k8sPrepareCredentialUpdate = (id, name, serverURL, authMode) => {
      const idEl = document.getElementById('k8s-id');
      const nameEl = document.getElementById('k8s-name');
      const urlEl = document.getElementById('k8s-url');
      const authEl = document.getElementById('k8s-auth');
      const credentialEl = document.getElementById('k8s-credential');
      const submit = document.getElementById('k8s-submit');
      const msg = document.getElementById('k8s-msg');
      if (idEl) idEl.value = id || '';
      if (nameEl) nameEl.value = name || '';
      if (urlEl) urlEl.value = serverURL || '';
      if (authEl) authEl.value = authMode || 'kubeconfig';
      if (credentialEl) {
        credentialEl.value = '';
        credentialEl.focus();
      }
      if (submit) submit.textContent = '인증 갱신';
      if (msg) msg.innerHTML = '<span class="status warn">인증 갱신 모드</span> 새 kubeconfig/token을 붙여 넣고 저장하세요. minikube TLS 오류는 <code>kubectl config view --raw --minify --flatten</code> 출력으로 갱신하면 됩니다.';
      const cardEl = nameEl && nameEl.closest('section');
      if (cardEl) cardEl.scrollIntoView({ behavior: 'smooth', block: 'start' });
    };
    window.k8sRegister = async () => {
      const msg = document.getElementById('k8s-msg');
      const auth = document.getElementById('k8s-auth').value;
      const credential = document.getElementById('k8s-credential').value;
      const body = { name: document.getElementById('k8s-name').value, server_url: document.getElementById('k8s-url').value, auth_mode: auth };
      const id = (document.getElementById('k8s-id') && document.getElementById('k8s-id').value || '').trim();
      if (id && auth !== 'in_cluster' && !credential.trim()) {
        if (msg) msg.innerHTML = '<span class="status error">인증 갱신에는 새 kubeconfig/token이 필요합니다.</span>';
        return;
      }
      if (id) body.id = id;
      if (auth === 'token' || auth === 'service_account') body.token = credential; else body.kubeconfig = credential;
      try {
        const d = await api('/admin/k8s/clusters', { method: 'POST', body: JSON.stringify(body) });
        if (msg) msg.innerHTML = '<span class="status">' + (id ? '갱신됨' : '등록됨') + '</span> ' + escapeHTML(d.cluster && d.cluster.id || '');
        await renderK8sOperations();
      } catch (e) { if (msg) msg.innerHTML = '<span class="status error">' + escapeHTML(e.message) + '</span>'; }
    };
    window.k8sTest = async (id) => {
      try { await api('/admin/k8s/clusters/' + encodeURIComponent(id) + '/test', { method: 'POST', body: '{}' }); }
      catch (e) { alert(e.message); }
      await renderK8sOperations();
    };
    window.k8sCollect = async (id) => {
    ...
    async function renderK8sTimeline(params) {
      const view = document.getElementById('view');
      ...
      view.innerHTML = section('K8s 변경 타임라인', '<div class="empty">불러오는 중...</div>');

      let clusters;
      try { clusters = await api('/admin/k8s/clusters'); }
      catch (e) { view.innerHTML = section('K8s 변경 타임라인', '<div class="card-body" style="padding:16px"><p class="muted">' + escapeHTML(e.message) + '</p></div>'); return; }
      ...
      let timeline = { entries: [] }, diffResp = { diff: null }, manifestResp = { yaml: '' };
      try {
        const calls = [api('/admin/k8s/timeline?' + qs.toString())];
        if (name) {
          calls.push(api('/admin/k8s/diff?' + qs.toString()));
          calls.push(kind ? api('/admin/k8s/manifest?' + qs.toString()).catch(() => ({ yaml: '' })) : Promise.resolve({ yaml: '', note: 'kind를 지정하면 현재 manifest가 표시됩니다.' }));
        } else {
          calls.push(Promise.resolve({ diff: null, note: '리소스 이름을 지정하면 직전 변경 Diff가 표시됩니다.' }));
          calls.push(Promise.resolve({ yaml: '' }));
        }
        [timeline, diffResp, manifestResp] = await Promise.all(calls);
      } catch (e) {
        view.innerHTML = section('K8s 변경 타임라인', '<div class="card-body" style="padding:16px"><p class="muted">' + escapeHTML(e.message) + '</p></div>');
        return;
      }

      const filterBar =
        '<div class="card-body"><div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">' +
        '<select id="k8stl-cluster">' + clusterOpts + '</select>' +
        '<input id="k8stl-ns" placeholder="namespace" value="' + escapeAttr(ns) + '" style="min-width:160px">' +
        '<input id="k8stl-name" placeholder="리소스 이름" value="' + escapeAttr(name) + '" style="min-width:200px">' +
        '<input id="k8stl-kind" placeholder="kind (예: Deployment)" value="' + escapeAttr(kind) + '" style="min-width:160px">' +
        '<button type="button" onclick="k8sTimelineGo()">조회</button>' +
        '</div><div class="muted" style="font-size:11px;margin-top:4px">배포·scale·pod 재생성·이벤트·액션을 한 시간축으로 보여줍니다. 민감 env/secret 값은 Diff에서 자동 마스킹됩니다.</div></div>';

      const sevClass = (s) => s === 'critical' ? 'error' : (s === 'warning' ? 'warn' : '');
      const catLabel = (c) => ({ revision: '변경', event: '이벤트', action: '액션' })[c] || c;
      const entries = timeline.entries || [];
      const tlBody = entries.length ? entries.map(e =>
        '<div style="display:flex;gap:10px;padding:8px 4px;border-bottom:1px solid var(--line)">' +
        '<div class="muted" style="font-size:11px;min-width:140px">' + ago(e.at) + '</div>' +
        '<div style="min-width:54px"><span class="status ' + sevClass(e.severity) + '" style="font-size:10px">' + escapeHTML(catLabel(e.category)) + '</span></div>' +
        '<div style="flex:1"><strong>' + escapeHTML(e.title || '-') + '</strong> <span class="muted" style="font-size:11px">' + escapeHTML((e.kind || '') + '/' + (e.name || '')) + '</span>' +
        (e.detail ? '<div class="muted" style="font-size:11px">' + escapeHTML(e.detail) + '</div>' : '') + '</div></div>'
      ).join('') : '<div class="muted" style="padding:12px">표시할 타임라인 항목이 없습니다.</div>';

      const d = diffResp.diff;
      let diffCard;
      if (d && (d.changes || []).length) {
        const hi = (d.highlights || []).map(h => '<span class="status warn" style="font-size:10px;margin-right:4px">' + escapeHTML(h) + '</span>').join('');
        const rows = d.changes.map(c =>
          '<tr><td style="font-size:11px;word-break:break-all">' + escapeHTML(c.path) + (c.highlight ? ' <span class="status" style="font-size:9px">' + escapeHTML(c.highlight) + '</span>' : '') + '</td>' +
          '<td class="muted" style="font-size:11px;word-break:break-all">' + escapeHTML(c.old || '∅') + '</td>' +
          '<td style="font-size:11px;word-break:break-all">' + escapeHTML(c.new || '∅') + '</td>' +
          '<td>' + escapeHTML(c.kind) + '</td></tr>').join('');
        diffCard = card('직전 변경 Diff (' + relativeTime(d.from_observed_at) + ' → ' + relativeTime(d.to_observed_at) + ')',
          '<div class="card-body"><div style="margin-bottom:6px">' + (hi || '<span class="muted" style="font-size:11px">주요 필드 변경 없음</span>') + '</div>' +
          '<table><thead><tr><th>필드</th><th>이전</th><th>이후</th><th>구분</th></tr></thead><tbody>' + rows + '</tbody></table></div>');
      } else {
        diffCard = card('직전 변경 Diff',
          '<div class="card-body"><p class="muted" style="font-size:12px">' + escapeHTML(diffResp.note || '변경 Diff가 없습니다.') + '</p></div>');
      }

      let manifestCard = '';
      if (manifestResp && manifestResp.yaml) {
        manifestCard = card('현재 Manifest (민감값 마스킹)',
          '<div class="card-body"><pre style="margin:0;max-height:420px;overflow:auto;background:var(--panel-alt);padding:10px;border-radius:6px;font-size:11px;white-space:pre">' + escapeHTML(manifestResp.yaml) + '</pre></div>');
      } else if (name && manifestResp && manifestResp.note) {
        manifestCard = card('현재 Manifest', '<div class="card-body"><p class="muted" style="font-size:12px">' + escapeHTML(manifestResp.note) + '</p></div>');
      }
      ...
      view.innerHTML =
        ...
        diffCard +
        manifestCard +
        card('타임라인', '<div class="card-body">' + tlBody + '</div>');
    }
    window.k8sTimelineGo = () => {
      const qs = new URLSearchParams();
      const cl = document.getElementById('k8stl-cluster').value;
      const ns = document.getElementById('k8stl-ns').value.trim();
      const name = document.getElementById('k8stl-name').value.trim();
      const kind = document.getElementById('k8stl-kind').value.trim();
      if (cl) qs.set('cluster_id', cl);
      if (ns) qs.set('namespace', ns);
      if (name) qs.set('name', name);
      if (kind) qs.set('kind', kind);
      location.hash = '#/k8s-timeline' + (qs.toString() ? '?' + qs.toString() : '');
    };

    // ---------- K8s 장애 분석 센터 (RCA-05~10) ----------
    async function renderK8sRCACenter(params) {
      const view = document.getElementById('view');
      const clusterId = (params && params.get('cluster_id')) || '';
      ...
      const filterBar =
        '<div class="card-body"><div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">' +
        '<select id="k8srca-cluster" onchange="k8sRCAGo()">' + clusterOpts + '</select>' +
        '<span class="muted" style="font-size:12px">' + escapeHTML(rca.note || '') + '</span></div></div>';

      const sevClass = (s) => s === 'critical' || s === 'high' ? 'error' : (s === 'medium' ? 'warn' : '');
      const list = (rca.candidates || []);
      // high severity first
      const order = { critical: 0, high: 1, medium: 2, low: 3 };
      list.sort((a, b) => (order[a.severity] ?? 9) - (order[b.severity] ?? 9));
      const cards = list.length ? list.map(c => {
        const tlHref = '#/k8s-timeline?' + new URLSearchParams({ cluster_id: c.cluster_id || '', namespace: c.namespace || '', name: c.resource_name || '', kind: c.resource_kind || '' }).toString();
        const evid = (c.evidence || []).length ? '<div style="margin-top:6px"><strong style="font-size:11px">근거</strong><ul style="margin:2px 0 0;padding-left:16px">' + (c.evidence || []).map(x => '<li class="muted" style="font-size:11px">' + escapeHTML(x) + '</li>').join('') + '</ul></div>' : '';
        const checks = (c.check_resources || []).length ? '<div style="margin-top:6px"><strong style="font-size:11px">점검 대상</strong> <span class="muted" style="font-size:11px">' + escapeHTML((c.check_resources || []).join(' · ')) + '</span></div>' : '';
        const acts = (c.actions || []).length ? '<div style="margin-top:6px"><strong style="font-size:11px">조치 후보</strong><ul style="margin:2px 0 0;padding-left:16px">' + (c.actions || []).map(x => '<li style="font-size:11px">' + escapeHTML(x) + '</li>').join('') + '</ul></div>' : '';
        return card(
          (c.severity || '-') + ' ' + (c.condition || '') + ' — ' + ((c.namespace || '-') + '/' + c.resource_kind + '/' + c.resource_name),
          '<div class="card-body"><p style="margin:0 0 4px"><span class="status ' + sevClass(c.severity) + '" style="font-size:10px">' + escapeHTML(c.severity) + '</span> ' + escapeHTML(c.cause || '') + '</p>' + evid + checks + acts +
    ...
    async function renderK8sConnectivity(params) {
      const view = document.getElementById('view');
      const clusterId = (params && params.get('cluster_id')) || '';
      view.innerHTML = section('K8s 연결성 점검', '<div class="empty">불러오는 중...</div>');
      let clusters, conn;
      try {
        [clusters, conn] = await Promise.all([
          api('/admin/k8s/clusters'),
          api('/admin/k8s/connectivity' + (clusterId ? '?cluster_id=' + encodeURIComponent(clusterId) : '')),
        ]);
      } catch (e) {
        view.innerHTML = section('K8s 연결성 점검', '<div class="card-body" style="padding:16px"><p class="muted">' + escapeHTML(e.message) + '</p></div>');
        return;
      }
      const clusterOpts = '<option value="">전체 클러스터</option>' + (clusters.clusters || []).map(cl =>
        '<option value="' + escapeAttr(cl.id) + '"' + (cl.id === clusterId ? ' selected' : '') + '>' + escapeHTML(cl.name) + '</option>').join('');
      const filterBar =
        '<div class="card-body"><div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">' +
        '<select id="k8sconn-cluster" onchange="k8sConnGo()">' + clusterOpts + '</select>' +
        '<span class="muted" style="font-size:12px">' + escapeHTML(conn.note || '') + '</span></div></div>';

      const sevClass = (s) => s === 'critical' || s === 'high' ? 'error' : (s === 'medium' ? 'warn' : '');
      const groups = { Service: [], Ingress: [], PersistentVolumeClaim: [] };
      (conn.findings || []).forEach(f => { (groups[f.resource_kind] || (groups[f.resource_kind] = [])).push(f); });
      const rowsFor = (arr) => arr.length ? arr.map(f =>
        '<tr><td><span class="status ' + sevClass(f.severity) + '" style="font-size:10px">' + escapeHTML(f.severity) + '</span></td>' +
        '<td>' + escapeHTML(f.check) + '</td>' +
        '<td>' + escapeHTML((f.namespace || '-') + '/' + f.resource_name) + '</td>' +
        '<td class="muted" style="font-size:11px">' + escapeHTML(f.message || '') + '<br>' + escapeHTML((f.evidence || []).join(' · ')) + '</td>' +
        '<td><a href="#/k8s-timeline?' + escapeAttr(new URLSearchParams({ cluster_id: f.cluster_id || '', namespace: f.namespace || '', name: f.resource_name || '', kind: f.resource_kind || '' }).toString()) + '">타임라인</a></td></tr>'
      ).join('') : '<tr><td colspan="5" class="muted">문제 없음.</td></tr>';
      const tbl = (title, arr) => card(title + ' (' + arr.length + ')',
        '<div class="card-body"><table><thead><tr><th>심각도</th><th>점검</th><th>리소스</th><th>내용</th><th></th></tr></thead><tbody>' + rowsFor(arr) + '</tbody></table></div>');

      const all = conn.findings || [];
      view.innerHTML =
        section('K8s 연결성 점검', '<div class="kpis">' +
          kpi('총 이슈', fmt(all.length)) +
          kpi('high', fmt(all.filter(x => x.severity === 'high' || x.severity === 'critical').length)) + '</div>') +
        card('필터', filterBar) +
        tbl('Service 연결성', groups.Service || []) +
        tbl('Ingress', groups.Ingress || []) +
        tbl('PVC', groups.PersistentVolumeClaim || []);
    }
    window.k8sConnGo = () => {
      const cl = document.getElementById('k8sconn-cluster').value;
      location.hash = '#/k8s-conn' + (cl ? '?cluster_id=' + encodeURIComponent(cl) : '');
    };
    ...
    async function renderK8sActions(params) {
      ...
      const pending = acts.filter(a => a.status === 'pending' || a.status === 'approval_required');
      const rows = acts.length ? acts.map(a => {
        const canDecide = a.status === 'pending' || a.status === 'approval_required';
        const btns = canDecide
          ? '<button type="button" class="secondary" onclick="k8sActDecide(\'' + escapeAttr(a.id) + '\',\'approve\')">승인</button> ' +
            '<button type="button" class="secondary" onclick="k8sActDecide(\'' + escapeAttr(a.id) + '\',\'reject\')">반려</button>'
          : (a.status === 'approved'
            ? '<button type="button" onclick="k8sActExecute(\'' + escapeAttr(a.id) + '\')">실행</button>'
            : '<span class="muted" style="font-size:11px">' + escapeHTML(a.approved_by || a.executed_by || '-') + '</span>');
        return '<tr><td>' + escapeHTML(a.action) + '</td>' +
          '<td>' + escapeHTML((a.namespace || '-') + '/' + a.resource_kind + '/' + a.resource_name) + '</td>' +
      ...
      view.innerHTML =
        ...
        card('필터', filterBar) +
        card('액션 요청',
          '<div class="card-body"><div class="muted" style="font-size:11px;margin-bottom:6px">실제 클러스터 실행기는 아직 연결되지 않았습니다. 승인/반려 워크플로우와 영향도 검토만 수행됩니다.</div>' +
    ...
    window.k8sActDecide = async (id, command) => {
      let body = '{}';
      if (command === 'reject') {
        const reason = prompt('반려 사유를 입력하세요(선택):') || '';
        body = JSON.stringify({ result: reason });
      }
      try { await api('/admin/k8s/actions/' + encodeURIComponent(id) + '/' + command, { method: 'POST', body }); }
      catch (e) { alert(e.message); }
      await renderK8sActions(new URLSearchParams(location.hash.split('?')[1] || ''));
    };
    window.k8sActExecute = async (id) => {
      if (!confirm('이 액션을 실제 클러스터에 실행할까요? (되돌릴 수 없는 변경일 수 있습니다)')) return;
      try { await api('/admin/k8s/actions/' + encodeURIComponent(id) + '/execute', { method: 'POST', body: '{}' }); }
      catch (e) { alert(e.message); }
      await renderK8sActions(new URLSearchParams(location.hash.split('?')[1] || ''));
    };
    ...
    async function renderK8sSecurity(params) {
      ...
      const psRows = (r.pod_security || []).filter(p => p.level !== 'restricted').map(p =>
        ...
        '<td>' + escapeHTML((p.namespace || '-') + '/' + p.kind + '/' + p.name) + '</td>' +
        '<td class="muted" style="font-size:11px">' + escapeHTML((p.violations || []).join(' · ')) + '</td></tr>').join('')
        || '<tr><td colspan="3" class="muted">restricted 미만 워크로드 없음.</td></tr>';

      const findTable = (title, arr) => {
        const rows = (arr || []).length ? (arr || []).map(f =>
          '<tr><td><span class="status ' + sevClass(f.severity) + '" style="font-size:10px">' + escapeHTML(f.severity) + '</span></td>' +
          '<td>' + escapeHTML(f.rule) + '</td>' +
          '<td>' + escapeHTML((f.namespace || '-') + '/' + f.resource_kind + '/' + f.resource_name) + '</td>' +
          '<td class="muted" style="font-size:11px">' + escapeHTML(f.message + ' ' + (f.evidence || []).join(', ')) + '</td></tr>').join('')
          : '<tr><td colspan="4" class="muted">없음.</td></tr>';
        return card(title, '<div class="card-body"><table><thead><tr><th>심각도</th><th>규칙</th><th>리소스</th><th>내용</th></tr></thead><tbody>' + rows + '</tbody></table></div>');
      };
      ...
      view.innerHTML =
        section('K8s 보안', '<div class="kpis">' +
          ...
          kpi('Restricted', fmt(sum.restricted || 0)) +
        ...
        (function () {
          ...
          const rows = es.length ? es.map(e =>
            '<tr><td>' + escapeHTML((e.namespace || '-') + '/' + e.kind + '/' + e.name) + '</td>' +
            '<td class="muted" style="font-size:11px">' + escapeHTML(ago(e.to_observed_at)) + '</td>' +
        ...
        (function () {
          ...
          if (!an.length) return '';
          const rows = an.map(a => '<tr><td>' + escapeHTML(a.resource_name) + '</td><td class="muted" style="font-size:11px">' + escapeHTML(a.message) + '</td></tr>').join('');
          return card('감사 이상 (SEC-09)', '<div class="card-body"><table><thead><tr><th>요청자</th><th>내용</th></tr></thead><tbody>' + rows + '</tbody></table></div>');
        })() +
        (function () {
          const tls = (data && data.tls) || [];
          if (!tls.length) return '';
          const rows = tls.map(c =>
            '<tr><td><span class="status ' + sevClass(c.severity) + '" style="font-size:10px">' + escapeHTML(c.severity) + '</span></td>' +
            '<td>' + escapeHTML((c.namespace || '-') + '/' + c.secret) + '</td>' +
    ...
    async function renderK8sSettings() {
      ...
      view.innerHTML =
        ...
        card('비용 단가',
          ...
          '<input id="set-mem" type="number" value="' + escapeAttr(String(pr.mem_gb_monthly_krw || 0)) + '" style="width:140px"> <span class="muted" style="font-size:11px">KRW/GB·월</span>' +
    ...
    window.k8sSettingsSaveCost = async () => {
      ...
      try {
        await api('/admin/k8s/cost/config', { method: 'POST', body: JSON.stringify({
          cpu_core_monthly_krw: parseFloat(document.getElementById('set-cpu').value) || 0,
          mem_gb_monthly_krw: parseFloat(document.getElementById('set-mem').value) || 0 }) });
        msg.innerHTML = '<span class="status">단가 저장됨</span>';
      } catch (e) { msg.innerHTML = '<span class="status error">' + escapeHTML(e.message) + '</span>'; }
    };
    window.k8sSettingsSaveNoti = async () => {
      const msg = document.getElementById('set-msg');
      try {
    ...
    async function renderK8sPolicy() {
      ...
      const compRows = (comp.violations || []).length ? (comp.violations || []).map(v =>
        ...
        '<td>' + escapeHTML(v.rule_type) + '</td><td>' + escapeHTML((v.namespace || '-') + '/' + v.kind + '/' + v.name) + '</td>' +
      ...
      view.innerHTML =
        ...
        card('Admission 시뮬레이터 (SEC-05)',
          ...
          '<input id="sim-kind2" placeholder="kind (예: Deployment)" style="min-width:160px;margin-bottom:4px"><br>' +
    ...
    window.k8sPolicySimulate = async () => {
      ...
      let spec;
      try { spec = JSON.parse(document.getElementById('sim-spec').value || '{}'); }
      catch (e) { out.innerHTML = '<span class="status error">spec JSON 파싱 오류</span>'; return; }
      try {
        const d = await api('/admin/k8s/policies/simulate', { method: 'POST', body: JSON.stringify({ kind: document.getElementById('sim-kind2').value.trim(), spec }) });
        const badge = d.decision === 'deny' ? '<span class="status error">DENY</span>' : '<span class="status">ALLOW</span>';
        const rows = (d.results || []).map(r => '<li class="' + (r.violated ? '' : 'muted') + '" style="font-size:11px">' + (r.violated ? '❌' : '✅') + ' [' + escapeHTML(r.action) + '] ' + escapeHTML(r.rule_type) + (r.detail ? ' — ' + escapeHTML(r.detail) : '') + '</li>').join('');
        out.innerHTML = '<div style="margin-bottom:4px">결과: ' + badge + '</div><ul style="margin:0;padding-left:16px">' + (rows || '<li class="muted">활성 정책 없음</li>') + '</ul>';
    ...
    async function renderK8sCapacity(params) {
      ...
      const hpaRows = (r.hpas || []).length ? (r.hpas || []).map(h =>
        '<tr><td>' + escapeHTML((h.namespace || '-') + '/' + h.name) + '</td>' +
        '<td>' + escapeHTML(h.target_kind + '/' + h.target_name) + '</td>' +
        ...
        : '<tr><td colspan="5" class="muted">HPA가 없습니다.</td></tr>';
      ...
      const allocRows = (r.allocation || []).length ? (r.allocation || []).map(a =>
        ...
        '<td>' + escapeHTML((a.namespace || '-') + '/' + a.name) + '</td>' +
      ...
      view.innerHTML =
        ...
        (function () {
          ...
          const rows = ps.length ? ps.map(p =>
            '<tr><td>' + escapeHTML(p.node) + '</td><td>' + fmt(p.current_cpu_m) + 'm / ' + fmt(p.allocatable_cpu_m) + 'm</td>' +
            '<td>' + fmt(p.daily_growth_cpu_m) + 'm/일</td>' +
            '<td>' + (p.days_to_full < 0 ? '<span class="muted">증가 없음</span>' : '<span class="status ' + (p.days_to_full <= 7 ? 'error' : (p.days_to_full <= 30 ? 'warn' : '')) + '" style="font-size:10px">' + fmt(p.days_to_full) + '일</span>') + '</td></tr>').join('')
            : '<tr><td colspan="4" class="muted">추세 데이터 부족(2개 이상 샘플 필요).</td></tr>';
    ...
    window.k8sSimScale = async () => {
      const out = document.getElementById('sim-out');
      const cl = document.getElementById('k8scap-cluster').value;
      const qs = new URLSearchParams({ cluster_id: cl, kind: document.getElementById('sim-kind').value.trim(),
        namespace: document.getElementById('sim-ns').value.trim(), name: document.getElementById('sim-name').value.trim(),
        replicas: document.getElementById('sim-replicas').value.trim() });
      try {
        const d = await api('/admin/k8s/capacity/simulate?' + qs.toString());
        const s = d.simulation || {};
        out.innerHTML = 'replica ' + fmt(s.current_replicas) + ' → ' + fmt(s.target_replicas) +
    ...
    async function renderK8sCost(params) {
      ...
      view.innerHTML =
        ...
        lineTable('비용센터별', r.by_cost_center);
    }
    window.k8sCostGo = () => {
      const cl = document.getElementById('k8scost-cluster').value;
      location.hash = '#/k8s-cost' + (cl ? '?cluster_id=' + encodeURIComponent(cl) : '');
    };
    window.k8sCostSave = async () => {
      try {
        await api('/admin/k8s/cost/config', { method: 'POST', body: JSON.stringify({
    ...
    async function renderK8sAI(params) {
      const view = document.getElementById('view');
      const clusterId = (params && params.get('cluster_id')) || '';
      let clusters;
      try { clusters = await api('/admin/k8s/clusters'); }
      catch (e) { view.innerHTML = section('K8s AI 분석', '<div class="card-body" style="padding:16px"><p class="muted">' + escapeHTML(e.message) + '</p></div>'); return; }
      const clusterOpts = '<option value="">전체 클러스터</option>' + (clusters.clusters || []).map(cl =>
        '<option value="' + escapeAttr(cl.id) + '"' + (cl.id === clusterId ? ' selected' : '') + '>' + escapeHTML(cl.name) + '</option>').join('');
      view.innerHTML =
        ...
        card('자연어 장애 질문 (AI-OPS-01)',
          ...
          '<input id="ai-ns" placeholder="namespace(선택)" style="min-width:120px">' +
          '<input id="ai-kind" placeholder="kind(선택, 예: Pod)" style="min-width:120px">' +
          '<input id="ai-name" placeholder="리소스 이름(선택)" style="min-width:160px"></div>' +
          '<textarea id="ai-q" rows="2" placeholder="예: 왜 이 Pod가 죽었어?" style="width:100%"></textarea>' +
          '<div style="margin-top:6px"><button type="button" onclick="k8sAIAsk()">질문</button> ' +
          '<button type="button" class="secondary" onclick="k8sAIReport()">운영 리포트 생성</button></div>' +
          '<div id="ai-out" style="margin-top:10px"></div></div>');
    }
    function k8sAIRender(d) {
      const out = document.getElementById('ai-out');
      const text = d.answer || d.report || '';
      const ev = (d.evidence || []).map(e => '<li class="muted" style="font-size:11px">' + escapeHTML(e) + '</li>').join('');
      out.innerHTML =
        (d.llm_available === false ? '<div class="status warn" style="font-size:11px;margin-bottom:6px">LLM 미구성 — 근거만 표시</div>' : '') +
        (text ? '<div style="white-space:pre-wrap;background:var(--panel-alt);padding:10px;border-radius:6px;margin-bottom:8px">' + escapeHTML(text) + '</div>' : '') +
        (d.note ? '<div class="muted" style="font-size:11px;margin-bottom:6px">' + escapeHTML(d.note) + '</div>' : '') +
        '<details><summary style="cursor:pointer;font-size:12px">근거 ' + (d.evidence || []).length + '건</summary><ul style="margin:4px 0 0;padding-left:16px">' + (ev || '<li class="muted">없음</li>') + '</ul></details>';
    }
    window.k8sAIAsk = async () => {
      const out = document.getElementById('ai-out');
      out.innerHTML = '<div class="muted">분석 중...</div>';
      try {
        const d = await api('/admin/k8s/ai/ask', { method: 'POST', body: JSON.stringify({
          cluster_id: document.getElementById('ai-cluster').value,
          namespace: document.getElementById('ai-ns').value.trim(),
          kind: document.getElementById('ai-kind').value.trim(),
          name: document.getElementById('ai-name').value.trim(),
          question: document.getElementById('ai-q').value.trim() }) });
        k8sAIRender(d);
      } catch (e) { out.innerHTML = '<div class="status error">' + escapeHTML(e.message) + '</div>'; }
    };
    window.k8sAIReport = async () => {
      const out = document.getElementById('ai-out');
      out.innerHTML = '<div class="muted">리포트 생성 중...</div>';
      const cl = document.getElementById('ai-cluster').value;
      try {
    ...
    async function renderK8sMeta() {
      const view = document.getElementById('view');
      view.innerHTML = section('K8s 그룹·오너십', '<div class="empty">불러오는 중...</div>');
      let groups, clusters, ownership;
      try {
        [groups, clusters, ownership] = await Promise.all([
          api('/admin/k8s/groups'), api('/admin/k8s/clusters'), api('/admin/k8s/ownership'),
        ]);
      } catch (e) {
        view.innerHTML = section('K8s 그룹·오너십', '<div class="card-body" style="padding:16px"><p class="muted">' + escapeHTML(e.message) + '</p></div>');
        return;
      }
      const grpRows = (groups.groups || []).length ? (groups.groups || []).map(g =>
      ...
      view.innerHTML =
        ...
        card('네임스페이스 오너십 추가',
          ...
          '<input id="own-owner" placeholder="담당자" style="min-width:100px">' +
    ...
    async function renderMCP(initial) {
      ...
      const upstreamTable = upstreams.length ?
        ...
        : '<div class="empty">등록된 업스트림 MCP 서버 없음. 등록하면 Clustara 엔드포인트 <code>/mcp</code> 가 모든 서버의 도구를 <code>&lt;이름&gt;__&lt;도구&gt;</code> 로 합쳐 제공합니다.</div>';
      const gatewayHelp = '<div class="muted" style="font-size:12px; padding:0 14px 12px">' +
        '클라이언트(Claude Code·Cursor 등)를 Clustara <code>/mcp</code> 한 곳에만 연결하면 등록된 모든 MCP 서버의 도구를 함께 사용합니다. 도구·프롬프트는 <code>&lt;업스트림ID&gt;__&lt;이름&gt;</code> 로 네임스페이스되고, 호출은 위 정책(allowlist/차단)과 사용자 귀속·관측에 통합됩니다. (Streamable HTTP 업스트림 지원)' +
        '<br><strong>등록 확인</strong>: 각 행의 <em>테스트/도구</em> 버튼으로 연결·노출 도구를 즉시 확인. ' +
        '<br><strong>호출 검증(curl)</strong>: <code>curl -s http://&lt;gateway&gt;:9090/mcp -H "Authorization: Bearer pcg_..." -H "Content-Type: application/json" -d \'{"jsonrpc":"2.0","id":1,"method":"tools/list"}\'</code> → 집약된 도구 목록 JSON. ' +
        '<code>tools/call</code> 은 <code>{"method":"tools/call","params":{"name":"&lt;업스트림ID&gt;__&lt;도구&gt;","arguments":{}}}</code>. ' +
        '<br><strong>클라이언트 설정</strong>: MCP 서버 URL 을 <code>http://&lt;gateway&gt;:9090/mcp</code>, 헤더 <code>Authorization: Bearer &lt;발급키&gt;</code>.' +
        '</div>';
    ...
    function mcpTopologyView(topology) {
      ...
      const overflow = routeNodes.length > CAP
        ...
        : '';
      const colLbls = [
        '<text x="'+(C0+NW/2)+'" y="14" text-anchor="middle" font-size="9" fill="#94a3b8" font-family="system-ui">Clustara</text>',
      ...
      try {
        ...
        openModal('업스트림 테스트 — ' + escapeHTML(d.name || id),
          '<div style="padding:14px">' +
            ...
            (d.tools && d.tools.length ? '<div class="muted" style="font-size:12px">Clustara에서 호출 시 위 <code>네임스페이스</code> 이름을 사용하세요.</div>' : '') +
    ...
    // renderGatewayMCP shows the Clustara MCP Server catalog + a copyable client config.
    async function renderGatewayMCP() {
      const view = document.getElementById('view');
      view.innerHTML = section('Clustara MCP', '<div class="empty">불러오는 중...</div>');
      let d;
      try { d = await api('/admin/gateway-mcp/info'); }
      catch (e) { view.innerHTML = section('Clustara MCP', '<div class="card-body" style="padding:16px"><p class="muted">' + escapeHTML(e.message) + '</p></div>'); return; }
      ...
      view.innerHTML = section('Clustara MCP Server', '') +
        ...
        card('Tool 계약 (Contract Pack)', '<div class="card-body"><p class="muted" style="font-size:12px">각 Clustara MCP tool의 위험도·비용 정책·timeout·실행 여부·출력 스키마 계약입니다.</p><table><thead><tr><th>tool</th><th>위험</th><th>비용</th><th>timeout</th><th>유형</th><th>출력</th></tr></thead><tbody>' + contractRows + '</tbody></table></div>') +
    ...
    async function renderMCPContracts() {
      ...
      host.innerHTML = card('MCP Tool Contract Registry (' + cs.length + ')',
        '<div class="card-body"><p class="muted" style="font-size:12px">MCP tool의 입력/출력 스키마·위험등급·타임아웃·허용 역할·비용 정책·소유자를 계약으로 고정하고, 실제 Clustara tool과의 스키마 드리프트를 탐지합니다.</p>' +
    ...
    window.meActionModal = (key) => {
      if (key === 'safety') {
        openModal('안전 가이드 — 민감정보 다루기',
          ...
          '<li>운영 DB 자격증명이나 내부 시크릿을 붙여넣지 마세요. Clustara가 시크릿을 탐지·마스킹하지만, 입력하지 않는 것이 가장 안전합니다.</li>' +
          '<li>꼭 필요한 최소한의 맥락만 제공하고, 외부로 나가면 안 되는 자료는 사내 승인 경로를 따르세요.</li>' +
          '</ul>' +
          '<p class="muted" style="font-size:12px">이 Clustara는 Secret Firewall로 민감 패턴을 탐지/마스킹/차단하며, 위반 요청은 감사 로그에 기록됩니다.</p>' +
      ...
      if (key === 'model_switch') {
        openModal('전환 가이드 — 더 저렴한 모델로',
          ...
          '<ul style="margin:8px 0 8px 18px">' +
          '<li><strong>자동 라우팅 사용</strong>: 모델명을 <code>vibe/auto</code>로 지정하면 Clustara가 작업 난이도에 맞춰 비용·품질 균형 모델을 선택합니다.</li>' +
    ...
    async function renderSSOSettings() {
      ...
      const updated = c.source === 'db' && c.updated_at
        ? '<p class="muted" style="font-size:11px;margin-top:4px">최종 수정: ' + ago(c.updated_at) + (c.updated_by ? ' · ' + escapeHTML(c.updated_by) : '') + '</p>'
        : '';
    ...
    async function renderText2SQL() {
      ...
      document.getElementById('view').innerHTML =
        '<section><h2>Text2SQL</h2><div style="padding:0 14px 8px" class="muted">자연어 질문을 읽기 전용 SQL로 변환합니다. 사용자는 <code>vibe/text2sql-*</code> 가상 모델을 호출하고, Clustara가 내부적으로 실제 업스트림 모델을 선택해 SQL을 생성·검증·(선택)실행합니다.</div></section>' +
    ...
    function knowledgePanel(snippets) {
      ...
      return form + table +
        '<div class="muted" style="font-size:12px; padding:0 14px 12px">반복되는 규칙/시스템 프롬프트를 한 번 등록하면, 클라이언트는 본문 대신 <code>{{kb:ID}}</code> 참조만 보내거나 <code>X-Vibe-Knowledge: ID1,ID2</code> 헤더를 붙이면 됩니다. Clustara가 업스트림 전송 시 전체 텍스트로 확장합니다(응답 헤더 <code>X-Knowledge-Expanded</code>로 확인). 규칙을 한 곳에서 고치면 모든 호출에 즉시 반영되고, 클라이언트 페이로드·로그 저장이 줄어듭니다. 업스트림 토큰 비용은 provider 프리픽스 캐싱(cached 토큰)과 결합될 때 절감됩니다.</div>';
    }
