...
운영(운영 홈·클러스터·수집 상태·변경 타임라인·장애 분석·장애 워룸·리소스 그래프·연결성 점검·액션 승인함·용량·자동확장·그룹·오너십·AI 분석·리포트 센터·SLO 센터) · 비용 · 보안 · 정책 센터 · 운영 설정 · 설정.
...
3. 장애 후보는 **장애 워룸**에서 근거·영향도·변경 이력 확인
4. **리소스 그래프**와 **연결성 점검**으로 Service/Ingress/PVC 영향 범위와 이상 점검
5. 주간: **보안**(Pod Security·RBAC·TLS 만료)·**용량**(확장 한계·과다 할당)·**비용**·**SLO** 리뷰
6. **정책 센터**로 표준 정책 위반 추적, 배포 전 **Admission 시뮬레이터** 활용
