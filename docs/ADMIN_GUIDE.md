...
Kubernetes 비용 화면은 resource request 기반 월 기준 추정치와 최신 CPU·Memory 실사용에 30% 안정성 headroom을 적용한 조정 시나리오를 분리합니다. 24시간 누적 환산, 최근 30일 일별 스냅샷, 최근 12개월 월말 스냅샷 추세와 Namespace별 비용 비중을 제공하며, Metric·Request coverage로 추정 신뢰도를 표시합니다. 시간별 값은 실제 청구액이 아닌 월 추정치의 730시간 환산이고 스토리지·네트워크·LoadBalancer·GPU·라이선스·할인은 별도 비용임을 화면에 명시합니다.
