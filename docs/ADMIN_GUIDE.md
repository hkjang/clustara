...
Pod 목록·상세·로그·증적 번들을 한 화면에서 확인합니다. CrashLoop/OOM/ImagePull/Pending/Evicted 계열 Pod, restart가 많은 Pod, Warning 이벤트가 붙은 Pod를 빠르게 찾는 데 초점을 둡니다.
...
- API: `GET /admin/k8s/pods`, `GET /admin/k8s/pods/{namespace}/{pod}`, `GET /admin/k8s/pods/{namespace}/{pod}/logs`, `GET /admin/k8s/pods/{namespace}/{pod}/logs/stream`, `POST /admin/k8s/pods/{namespace}/{pod}/evidence-bundle`
