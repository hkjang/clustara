...
> **참고 — Clustara K8s 운영 허브.** 현재 어드민 UI 메뉴는 K8s 운영(운영 홈·클러스터·변경 타임라인·장애 분석·연결성 점검·액션 승인함·용량·자동확장·그룹/오너십·AI 분석·비용·보안·정책 센터·운영 설정)으로 구성됩니다. 이 문서에서 설명하는 일부 탭(대시보드·MCP·라우팅·프롬프트·Text2SQL 등)은 **내장 게이트웨이 코어**에 해당하며 기본 메뉴에서는 제외되어 있습니다(라우트는 유지). K8s 운영의 1차 문서는 **[K8s 운영 허브 가이드](K8S_OPERATIONS_HUB.md)** 입니다.
