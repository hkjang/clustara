...
Pod 목록·상세·로그·로그 분석·증적 번들·Golden Pod Diff·Health Replay를 한 화면에서 확인합니다. CrashLoop/OOM/ImagePull/Pending/Evicted 계열 Pod, restart가 많은 Pod, Warning 이벤트가 붙은 Pod를 빠르게 찾는 데 초점을 둡니다.
...
- Terminal Policy Builder: role, cluster, namespace glob, Pod label selector, 허용·차단 명령, 승인 필요, 최대 세션 시간, 감사 저장 여부를 설정합니다. 실제 Pod exec/web terminal 기능은 이 정책 평가를 통과한 뒤 승인 워크플로우로 연결됩니다.
- API: `GET/POST /admin/k8s/cost/config`, `/admin/k8s/notify/config`, `/admin/k8s/latency/config`, `/admin/notifications/mattermost`
- 터미널 정책 API: `GET/POST /admin/k8s/terminal-policies`, `DELETE /admin/k8s/terminal-policies/{id}`, `POST /admin/k8s/terminal-policies/evaluate`
