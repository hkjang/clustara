...
서비스 상세의 **백업 요청**에서는 PostgreSQL 논리 백업과 CSI VolumeSnapshot을 선택할 수 있습니다. 성공한 논리 백업의 **복구 미리보기**는 원본 또는 별도 대상 ServiceInstance의 호환성·PVC·Secret·Service 관측 여부와 데이터 변경 영향을 검사합니다. `readyToUse` 스냅샷은 새 PVC 이름·용량·StorageClass를 입력해 비파괴 Clone Restore를 요청할 수 있으며, 기존 PVC가 있으면 차단됩니다. 허용된 경우에만 Restore Job 또는 `dataSource: VolumeSnapshot` PVC 승인 요청을 만들고 백업/복구 원장에서 적용 및 완료 상태를 추적합니다. 클론 PVC의 워크로드 연결은 자동 변경하지 않으므로 별도 영향도 분석·승인 변경으로 수행해야 합니다.
