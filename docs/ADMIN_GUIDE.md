...
Kubernetes 비용 화면은 CPU·Memory·`nvidia.com/gpu` request와 PVC storage request 기반 월 기준 추정치, 최신 CPU·Memory 실사용에 30% 안정성 headroom을 적용한 조정 시나리오를 분리합니다. GPU/GPU·월, Disk GB·월 단가는 운영 환경에 맞게 설정할 수 있습니다. 24시간 누적 환산, 최근 30일 일별 스냅샷, 최근 12개월 월말 스냅샷 추세와 Namespace별 비용 비중을 제공하며, Metric·Request coverage로 추정 신뢰도를 표시합니다. 자동 스냅샷은 기본 활성·24시간 주기로 서버 시작 후 동작하고 클러스터별 Namespace 합계를 기록하며, 동일 날짜 재실행은 upsert됩니다. 수동 기록은 자동화 오류 복구용으로 유지합니다. 네트워크 egress·LoadBalancer·StorageClass별 차등·라이선스·약정 할인은 별도 비용입니다.
