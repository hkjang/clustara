...
`nodeKey`(클러스터+이름), Pod 메트릭은 그에 맞춘 `podMetricKey` 로 키잉합니다 — 같은 패키지의
`node_monitoring` 이 이미 쓰던 조인 방식이고, 이전에는 제품의 두 화면이 같은 노드를 다르게
계산하고 있었습니다.

이전 동작은 다음과 같았습니다. SCALE-07 노드 패킹과 SCALE-08 GPU 표는 노드를 이름으로만
키잉해 동명 노드 둘이 한 행으로 합쳐졌고(allocatable 은 인벤토리에서 마지막에 온 클러스터
것), `.spec.nodeName` 매칭이 그 행에 두 클러스터의 Pod 를 모두 더했습니다 — 1000m·5000m 을
쓰는 4코어 노드 두 개가 **"75% 패킹된 노드 하나"** 로 나오고 다른 클러스터의 노드는 표에서
사라졌습니다. SCALE-03/04 의 Pod 최신 메트릭 맵은 namespace/name 키라 newest-first 중복
제거가 한 클러스터의 샘플만 남긴 뒤 그것을 동명 Pod 전부의 request 와 비교했고, request
1000m 중 600m 을 쓰는 Pod 가 다른 클러스터 Pod 의 250m 때문에 `over_provisioned` 로
올라왔습니다. SCALE-05 노드 예측은 두 클러스터의 샘플을 한 추세선에 섞어(가장 오래된 샘플과
최신 샘플이 서로 다른 머신일 수 있습니다) 어느 실제 노드도 설명하지 않는 days-to-full 을
그렸습니다.

다섯 결과 타입에 `cluster_id` 가 `omitempty` 로 추가되어(v0.9.278 의 `SecFinding` 과 같은
방식) 각 행이 어느 클러스터를 설명하는지 말합니다. map 순회로 만들어져 요청마다 순서가
달라지던 두 노드 표는 cluster → node 순으로 정렬됩니다. 어드민 UI 의 노드 표도 packing 을
클러스터까지 포함해 조인하고(바로 옆 Pod 조인은 이미 클러스터를 넘기고 있었습니다), 용량
페이지의 YAML 딥링크는 전 클러스터 보기에서 빈 `cluster_id` 대신 행 자신의 클러스터를
씁니다.
...
`FROM` 절의 **모든** 소스가 allow-list 검사를 받습니다. 쉼표로 이어진 목록과 괄호로
묶인 조인의 첫 소스가 이전에는 추출되지 않아 검사를 우회했습니다.

권한 오버레이로 테이블 접근을 제한하고 있다면 이 버전으로 올리세요. 읽기 전용
DSN 으로는 완화되지 않는 문제입니다.

### 외부 링크 스킴 검사

어드민 화면이 외부 데이터를 클릭 가능한 링크로 표시할 때 `http`·`https`·`mailto`
와 상대·해시 링크만 허용합니다. 다른 스킴은 링크 대신 평문으로 표시됩니다.
HTML 이스케이프는 값이 속성 밖으로 나가는 것만 막고 스킴은 막지 못하기 때문입니다.

### 요청 본문 읽기 상한

`LIMITS_MAX_REQUEST_BYTES` 를 설정하면 과대 요청은 거절될 뿐 아니라 **읽기 자체가**
한도+1 바이트에서 멈춥니다. 이전에는 본문을 전부 버퍼링한 뒤 크기를 확인했기 때문에,
메모리 보호를 기대하고 이 값을 설정해도 보호되지 않았습니다.

인증은 본문 읽기보다 먼저 수행되므로 인증 없는 요청은 본문을 읽기 전에 거절됩니다.
한도를 설정하지 않으면(기본값 `0`) 동작은 이전과 같습니다.

### 프로덕션 스킬 가드레일

스킬을 `production` 으로 저장하려면 승격 경로든 upsert 경로든 필수 항목이 모두
채워져 있어야 합니다: 지침, 허용 모델, 허용 도구, 허용 팀, 일일 한도(>0).

미설정 가드레일은 실행 시점에 **제한 없음** 으로 해석되므로, 비워 둔 채 프로덕션에
올리면 모든 모델·도구·팀에서 한도 없이 실행됩니다. `draft`·`staging` 은 제약이
없습니다 — 스킬을 만들어 가는 단계입니다.

### exec 세션 승인은 제3자가

정책이 승인을 요구한 Pod 터미널 세션은 **요청자 본인이 승인할 수 없습니다**. 다른
관리자가 결정해야 하며, 요청자가 승인을 시도하면 `403` 입니다.

- 자기 요청 철회(reject)는 계속 허용합니다. 접근 권한을 얻는 행위가 아닙니다.
- `super_admin` 은 내장 break-glass 정책으로 승인 단계 자체를 거치지 않으므로
  영향을 받지 않습니다.
- 관리자 인증이 비활성이면 모든 호출자가 `anonymous` 라 분리할 신원이 없으므로
  규칙이 적용되지 않습니다. 이 통제를 쓰려면 인증을 활성화해야 합니다.

### 실행 직전 중단 안전성 재확인

롤아웃 precheck 는 요청 시점에 평가되지만, 승인 실행은 한참 뒤에 이뤄질 수 있습니다.
실행 직전에 다음 세 가지를 다시 확인하고 해당하면 실행을 차단합니다.
...
> 이전에는 통과하던 승인 실행이 위 조건에 해당하면 차단됩니다. Pod 비정상·replica
> 미충족은 실행 차단 대상이 **아닙니다** — 망가진 워크로드를 되살리려는 재시작을
> 막지 않기 위해서이며, 요청 시점의 차단 판정으로는 유지됩니다.

PDB 판정은 일치하는 모든 budget 중 가장 제한적인 것을 따르며, 차단 사유에 결정을
내린 budget 이름이 포함됩니다.

### 롤백 실행 주체 분리

자동 롤백 패치는 지속형 워커만 시작합니다. 롤아웃 상세·진행 조회 같은 읽기
엔드포인트는 관측한 진행 상태를 기록하되 클러스터를 변경하지 않습니다.
`rollout:view` 만 가진 사용자가 페이지를 여는 것으로 롤백이 실행되거나, 그 사용자가
롤백 실행자로 기록되는 일이 없습니다.
...
터미널 세션 리퍼와 ClickHouse fact 재시도 큐는 이제 고칠 수 없는 항목을 무한히
재시도하지 않습니다. 읽을 수 없는 heartbeat 를 가진 exec 세션은 저장된 값을
대상으로 하는 CAS 로 정리되고(살아 있는 소유자가 갱신하면 세션이 유지됩니다),
ClickHouse 가 영구히 거부하는 fact 배치는 10회 시도 후 사유와 함께 정리됩니다.

두 큐 모두 오래된 항목부터 처리하므로, 포기 조건이 없으면 고칠 수 없는 항목이
큐 맨 앞을 영구 점유해 뒤의 정상 항목을 굶깁니다.

### 자동 롤백 완료 보장

롤아웃 리컨실러는 primary status 가 `failed`·`timed_out` 이어도 남은 롤백 작업을
계속 처리합니다. 자동 롤백 요청, 롤백 진행 관측, 롤백 타임아웃 판정이 모두 워커
경로에서 동작하므로 브라우저를 닫아도 롤백이 `monitoring` 에 갇히지 않습니다.

due 큐가 선택하는 모든 롤아웃 상태는 실제로 그 상태를 바꿀 수 있는 분기로
이어져야 하며, 이 불변식은 회귀 테스트로 고정되어 있습니다.

### 서버 스케줄러 수명주기와 측정된 워커 상태

서버 프로세스가 시작하는 주기 스케줄러(인벤토리 수집, 노드 메트릭, 비용 스냅샷,
리포트 전달, 서비스 재조정, Text2SQL 리포트, 설정 폴링, ClickHouse fact 적재)는
이제 서버 수명주기에 묶입니다. 종료 시 함께 취소되고, `main` 이 DB 를 닫기 전에
종료를 기다립니다.

`GET /admin/ops/workers` 는 이들을 **측정된 상태**로 보고합니다. 설정으로 스스로
꺼진 스케줄러는 실행이 아니라 유휴로 표시되며, tick 수·처리량·마지막 성공·연속
실패가 함께 노출되고 `clustara_worker_*` 메트릭에도 포함됩니다. 스케줄러 목록과
자동 비활성 조건은 [운영 가이드 5.6](./OPERATIONS.md)을 참고하세요.
