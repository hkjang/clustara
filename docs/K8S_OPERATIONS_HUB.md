...
> **버전: v0.8.1** · 이 문서는 Clustara Kubernetes 운영 허브 API를 설명합니다. (바이너리 `AppVersion`과 최신 릴리즈 태그가 동일하게 정렬됩니다.)
...
| Restart Storm 탐지 — 같은 workload 다수 Pod 재시작/비정상 시 서비스 단위 장애로 묶어 경고(POD-RULE-06) | ✅ |
