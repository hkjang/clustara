...
- **API**: `GET/POST /admin/k8s/groups`, `GET/POST/DELETE /admin/k8s/groups/{id}`, `POST /admin/k8s/clusters/{id}/group`, `GET/POST /admin/k8s/ownership`, `DELETE /admin/k8s/ownership/{cluster_id}/{namespace}`. 인벤토리/이벤트/finding/RCA 필터에 `group_id`·`team` 파라미터 추가.
